...
## 訂正（2026-08-13 実装時に判明）

実装の過程で、この診断自体の誤りが2つ見つかった。**訂正する。**

1. **「performer / offers / eventStatus のコードが無い」は誤り。**
   生成コードは 2026-08-01 から存在していた（`948a4404`）。
   実際の原因はデータ側:
   - LINEUPS 621行中 **501行が ARTIST_ID を持たない名前だけの行**で、
     リンク必須の生成関数が全部捨てていた → 名前だけでも出すよう修正
   - TICKETURL があるのは 104開催回中 20件 → offers はその20件には出ていた
   - EDITIONS の STATUS が**規約外の `published` 36件・空欄55件** →
     eventStatus がほぼ出ていなかった（既定 EventScheduled を出すよう修正）
2. **「Lighthouse にしきい値なし」は誤り。**
   CLS の合格ライン（0.06）は `.lighthouserc.json` に存在した。
   無かったのはカテゴリ別（性能・A11y 等）の床で、そちらを追加した。

教訓: **「コードに無い」と断定する前に、生成コードを読む。**
