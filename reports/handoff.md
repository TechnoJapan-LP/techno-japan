...
## 2026-08-18 / Codex / 作業中（モバイル記事詳細・共通メニューの表示統一）

### 実施
- モバイル記事詳細の固定ヘッダー直下で「ALL STORIES」が重ならないよう、記事上部の安全域を調整。
- 本文と見出しがスマホ画面の右側を過度に余らせないよう、モバイル本文幅と見出しの折返しを調整。
- 共通JS/CSSのキャッシュバージョンを更新し、ページごとの古いヘッダー表示が残らないよう準備。

### コミット
- SHA: 未コミット
- push / rebase 状態: pull前に未コミット変更があるため、コミット後にrebase予定

### 検証
- `git diff --check`: 成功
- 実ブラウザでのモバイル表示確認: 修正後に実施予定
- 生成詳細ページ全件の確認: 未実施

### 変更したパターン
- 記事詳細のモバイル戻る導線・本文見出し幅: 1パターン
- 共通JS/CSSキャッシュバージョン: ハブページ6枚、詳細ページは再生成予定

### 未確認の類似パターン
- FESTIVAL / ARTIST / VENUE詳細のモバイルヘッダー: 未確認
- JA / EN各ページの共通メニュー: 未確認

### 次の担当への注意・判断待ち
- 詳細ページを最新コードで再生成し、ローカル実ブラウザで記事詳細・ハブ・JA/EN・PC/モバイルを確認してから公開判断する。
