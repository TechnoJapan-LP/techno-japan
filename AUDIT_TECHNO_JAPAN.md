...
#### 追記: 真犯人は `MAX_TOKENS_SUMMARY = 1000` だった（2026-08-10）

**翻訳（上限16000）は成功するのに、タイトル生成（上限1000）だけが空**という
報告で確定した。**差はこの数字だけ。**

`max_tokens` は「出力の上限」であって「本文の長さ」ではない。
モデルが考える過程を出力に含めると、**本文に辿り着く前に上限へ当たり、
本文が空で返る。**タイトル3案は短いから 1000 で足りる、という見積もりが
そもそも見当違いだった。

`MAX_TOKENS_SUMMARY` を 4000 にした。
**上限は枠であって、課金は実際に出力した分だけ。**余裕を持たせても費用は
増えない。逆に切り詰めると、この形で静かに壊れる。

`check_gas_ai.mjs` に、要約の上限が 1000 より大きいこと・翻訳の上限が
それより大きいことを検査する2件を追加（計30件）。

**教訓。**「短い出力だから上限も小さくてよい」は成り立たない。
上限を絞る理由が無いなら絞らない。
