...
### 9-59. VENUE 登録の効率化と、キャパシティの廃止（2026-08-09）

会場を増やしたい・キャパシティは事実確認が難しいのでやめたい、という方針から。
**提案の前に、実際に使えるかを測った。**

#### 会場登録は住所も座標も手入力だった

フェスには「施設名を入れて📍検索 → 住所と座標を自動取得」（`geocodeFromLocation`）が
あるのに、**会場側には無かった。** 会場は住所を手で入れ、そこから座標を引く前提。

しかもその住所検索は**日本語住所ではほぼ当たらない**（実測）:
...
対応: `geocodeFromLocation` を会場でも使えるようにした。
会場は `LOCATION` 列を持たない（名前そのものが施設名）ので、
`v` のときは `NAME` を施設名として使う。

#### ★ 検索結果をそのまま入れるのは危険だった

移植にあたり既存22件で当たり具合を測ったところ **4/10**。
外れた分を日本語名で引き直しても **1/6** で、しかも:
...
**当たったように見えて、まったく違う場所が入る。**
`applyHit` は確認せず入力欄へ書き込んでいた。
地図リンクも詳細ページの座標も狂うが、**数字なので目視では気づけない。**

対応: 日本の範囲（北緯20〜46 / 東経122〜154）に入らない結果は採用せず、
次の候補へ進む。フェス側の同じ経路にも同時に効く。

`scripts/check_cms_geocode.mjs`（新規、CI 組込）で3項目を見張る。

**教訓: 便利機能を移植するときは、移植元が正しく動いているかまで測る。**
フェス側は以前から同じ危険を持っていた。

#### キャパシティの廃止

使用箇所は6つ。5つは空なら自動的に消えるが、
**地図ページだけ「CAPACITY —」という見出しが残る**作りだった。

表示は会場詳細・一覧・トップ・地図から削除。
検索用データ（JSON-LD の `maximumAttendeeCapacity`）は
`v.capacity ?` で囲まれており空なら出ないので、
**既存データが残っている会場のために残した。**

#### トップページの会場をランダムに

キャパ降順だったため、**同じ会場しかトップに出なかった。**
画像がある会場を優先し、その中はランダムに並べる。
まだ知られていない会場に出会う入口にする（利用者の提案）。

実測: 3回読み込んで3回とも違う並び。件数は8件で変わらない。
ランダムは描画時に1回だけで、スクロール中に入れ替わることはない。

#### 現状のデータ

```
VENUES 22件。NAME / CITY / AREA / TYPE / IMAGE / ADDRESS / 座標 /
INSTAGRAM / 説明文（日英）は22件すべて埋まっている。
未入力: GENRE 2件（WOMB / UNIT）、URL 1件（bonobo）
エリア: 渋谷8 / 心斎橋4 / その他各1。名古屋・福岡・札幌はゼロ
```

**`circus-tokyo` の AREA が `SHIBUAYA`（誤記）。**
会場ページに「SHIBUAYA, TOKYO」と表示され、渋谷で絞り込んでも出てこない。
データ側の修正が要る（勝手に直さず報告。AGENTS.md）。

#### 追記: 貼り付け用 TSV の列ズレを、貼る前に検出する（2026-08-09）

VENUE の下書きを **18列**で作ったが、シートは **24列**だった。
16列目以降がズレており、そのまま貼れば
`ogImage` の列に `name_en` が入るなどして値が混ざっていた。

**貼る直前にたまたま確認したから気づいた。** 列ズレは
エラーを出さないので、公開されるまで分からない
（HACHA MECHA / SPRING LOVE 春風 で実際に起きている。§9-9）。

`scripts/check_paste_tsv.mjs`（新規）で、貼る前に4点を見る:

```
列数がシートと同じか / 列名と順序が1つずつ一致するか /
各行のセル数がヘッダーと同じか / ID が既にシートにあるか
```

最後の1つは §9-58（EDITIONS の重複26行）の再発防止。
既にある ID を末尾に貼れば重複になる。

**自分で列を並べないこと。** シートの1行目をそのまま使えばズレようがない。
今回は「必要な列だけ書けばよい」と考えて18列にしたのが誤りだった。

*負のコントロール: 18列に戻すと列数違いを、既存IDを混ぜるとID重複を検出した。*

### 9-60. 会場詳細に Instagram が出ていなかった（2026-08-09）

会場追加を前にした事前調査（`reports/venue-audit-2026-08-09.md`）で見つけた。

**22件すべてに Instagram が入力されているのに、会場詳細ページで
一度も使われていなかった。**
...
一覧ページ（`venues.html`）では使われていたので、**詳細だけの漏れ**。
フェス詳細には Instagram アイコンを出す実装があり、会場だけ抜けていた。

クラブは公式サイトを持たない、あるいは更新が止まっている例が多く、
**Instagram が実質の一次情報**になっている。実際 `bonobo` は
公式サイトが無く Instagram だけ。出ていないのは機会損失だった。

#### 実装

公式サイトと同じ `<dl>` 内に並べた。フェスのようなアイコンではなく
テキストにしたのは、会場ページのこの欄が「タイプ / 住所 / 公式サイト」という
**項目名と値の一覧**で、そこにアイコンだけ混ざると浮くため。

URL をそのまま出すと長いので `@womb_tokyo` の形にする（`instagramHandle`）。
取り出せない形の URL はそのまま出して壊さない。

検索用データ（JSON-LD）の `sameAs` にも入れた。
公式サイトと Instagram を並べると、検索エンジンが同一の店だと判断しやすい。

```
sameAs: ["https://www.womb.co.jp/", "https://www.instagram.com/womb_tokyo/"]
```
...
### 9-63. STATUS / AUTHOR が本文エディタに覆われて押せなかった（2026-08-09）

記事フォームで **STATUS・AUTHOR・PUBLISH AT の入力欄が見えない**という報告。
実際に描画して矩形を測ると、原因は「隠れている」ではなく**重なり**だった。

```
                        top    bottom
エディタ枠(.ar-editor-wrap) 1751 〜 2289   ← 枠はここで終わる
本文(#ar-body-editor)      1824 〜 2362   ← 枠を 73px はみ出す
画像レイアウトツールバー      2370 〜 2426   ← 枠の完全に外
AUTHOR(#ar-authorId)      2384 〜 2422   ← ツールバーに覆われる
```

`.ar-editor-wrap` は `position:relative` だけで、中の子（本文＋ツールバー）の
高さを合算していなかった。**枠の高さが中身より小さい**ので、下に続く
PUBLISHING & SEO パネルが枠の終わりを基準に配置され、はみ出した
ツールバーの下に潜り込んでいた。**AUTHOR は見えないだけでなく押せない。**

#### 直し方を3案とも実測した

CSS の重なりは読んでも分からないので、順に当てて測った。

| 案 | 重なり | 本文の高さ | |
|---|---|---|---|
| 元の状態 | **あり** | 537px | |
| 1. `display:flex;flex-direction:column` | 解消 | **379px** | 本文が狭くなる ✗ |
| 2. 案1 + `#ar-body-editor{flex:1 0 auto}` | **再発** | 537px | 高さは戻るが重なる ✗ |
| 3. 案1 + `#ar-body-editor{min-height:540px}` | 解消 | 539px | **採用** ✅ |

flex にすると枠が中身を正しく包む代わりに、本文が余った高さに合わせて
縮む。`min-height` で下限を与えると、包みつつ従来の広さを保てる。
集中モード（`focus-mode`）は別の高さ規則を持つので `:not(.focus-mode)` で除外し、
ソース表示・プレビューを含む3モードでもツールバーが枠内に収まることを確認した。

#### 恒久チェック `scripts/check_cms_layout.mjs`

**CSS の重なりは静的検査では絶対に見つからない。**実際に描画して測るしかない。
headless Chrome で cms.html を開き、ツールバーと STATUS / AUTHOR / PUBLISH AT の
矩形が交差しないこと、本文の高さが 400px を下回らないことを見る。

ネガティブコントロールで、元の壊れた CSS に戻すと落ちることを確認済み。

作るときに踏んだ罠を2つ記録する。

- **認証の判定を `innerHTML` の文字列で見ない。** `/Access denied/` を
  `document.body.innerHTML` で探すと、cms.html のインライン script に
  書かれた文字列そのものに当たり、**正常時でも true** になった。
  差し替えの実態は「記事フォームが描画されないこと」なので、
  `#article-tab-form` の高さを直接見る。
- **測る位置は「開いた後」。** タブを `active` にする前に測ると
  `display:none` で 0 になる。
...
### 9-65. Publish が「順番待ちのまま」60分止まっていた（2026-08-09）

「Publish Now が完了しない」という報告。調べると、失敗ではなく**止まっていた。**

```
03:55  Publish pipeline  queued のまま65分。実行機が割り当てられない
04:24  Deploy            その後ろで pending のまま36分
```

GitHub 側の障害ではない（Actions / Pages とも All Systems Operational）。

#### なぜ気づけなかったか

3つが重なっている。

- **`timeout-minutes: 20` は効かない。**これは**動き出してからの時計**で、
  順番待ちのまま固まっている間は動かない。永久に待つ。
- **`concurrency: pages` で直列化している**（§9-50）。1本詰まると
  後続が全部止まる。実際、後ろのデプロイが36分待たされていた。
- **失敗ではないので通知が出ない。**赤いバツも付かない。
  ユーザーが「完了しない」と言うまで、誰も気づけない。

詰まった1本をキャンセルしたら、後ろで待っていたデプロイがすぐ通り、
CMS の修正（cms.css v=26）と JA ハブの更新が本番へ出た。
**外し方は分かっている。気づく手段が無かっただけ。**

#### 直し方: 見張り番を置く

詰まること自体は防げない（GitHub 側の都合）。**外す側を自動にする。**

`.github/workflows/unstick-queue.yml` を15分ごとに走らせ、
`scripts/unstick_ci_queue.mjs` が

1. 順番待ちのまま25分以上たった run を外す
2. それで pages グループが空になった場合だけ、デプロイを1回だけ蹴り直す

最悪でも40分で自動復旧する。

見張り番自身は `concurrency: pages` に**入れない。**入れると
**外したい相手の後ろに並んで一緒に固まる。**

#### 殺してはいけないもの

この手の自動化は、間違うと動いている CI を殺す。除外を明示する。

| 状態 | 扱い | 理由 |
|---|---|---|
| `in_progress` | **触らない** | 動いている。`timeout-minutes` が効く範囲 |
| `waiting` | **触らない** | **環境の承認待ち。**人の操作を待っている正常な状態 |
| しきい値未満 | 触らない | 通常の順番待ちは数秒〜数分 |
| `queued` / `pending` / `requested` かつ25分超 | 外す | |

`waiting` は現時点では発生しない（`github-pages` 環境の保護ルールは
branch_policy のみで、承認者の設定が無い）。**後から承認ルールを足したときに
事故る**ので、いま除外しておく。

#### 無限ループを防ぐ

蹴り直したデプロイがまた詰まると、放っておけば15分ごとに蹴り続ける。
**同じ SHA に対する `workflow_dispatch` 由来のデプロイが既にあれば蹴らず、
ジョブを失敗させて通知を出す。**繰り返し蹴っても同じなら、人が見るべき状態。

#### 試験

**この仕組みは普段は何もしないので、壊れていても気づけない。**
判断を純粋な関数 `decide()` に分け、`scripts/check_unstick_queue.mjs` で
13件を試験する。実際に起きた事故（65分の Publish ＋ 36分の Deploy）を
そのまま再現した事例を含む。CI 2本に組み込み済み。

ネガティブコントロール:

- `waiting` を対象に含めると落ちる（承認待ちを殺す誤り）
- 無限ループ防止を外すと落ちる

### 9-66. EDITIONS の重複1行で、Publish が丸1日失敗し続けていた（2026-08-09）

「Publish Now がエラーになる」という報告。原因は**データ1行**だった。

```
✗ EDITIONS: ID重複 "synapse-festival-2026"
エラーがあるためJSON書き出しを停止（スキーマ §6）
```

```
 92行目  LOCATION 空 / ADDRESS 空 / LAT 空          STATUS=published
107行目  INN THE PARK FUKUOKA / 住所・座標あり       STATUS=announced
```

92行目の `STATUS=published` は**開催回には存在しない値**
（EDITIONS.STATUS は `announced` / `on-sale` / `soldout` / `finished` /
`cancelled`。DATA_SCHEMA §273）。中身も空。**92行目が誤りで、107行目が正しい。**

#### いつからか

§9-58 の重複事故の**消し漏れ**だった。

```
08-08 02:47  ID重複 16件（big-fun-2025 / re-birth-festival-2026 / ultra-japan-2026 …）
             → ユーザーが削除
08-08 17:02  ID重複 1件（synapse-festival-2026）← 消し漏れ
08-09 05:19  ID重複 1件（同上）                  ← 丸1日、同じ理由で失敗し続けた
```

CMS 側の upsert（§9-58 で入れた、EDITION_ID が既にあれば末尾に足さずその行を
書き換える仕組み）は正しく動いている。**新しく増えたのではなく、
残っていたものが消えていなかった。**

#### なぜ1日気づけなかったか

**CMS は EDITIONS を取得していなかった。**

`publishNow` が取るのは `VENUES / FESTIVALS / ARTISTS / EVENTS / ARTICLES` だけ。
EDITIONS は data.js に入らないため取っていなかった。その結果:

- 押した時点では**成功したように見える**（差分確認も通り、コミットもされる）
- 失敗が分かるのは **20分後、CI が赤くなってから**
- しかも赤くなるのは GitHub 上で、CMS の画面には何も出ない

**「押せてしまう」ことが問題。**必ず失敗すると分かっている状態なら、
押す前に止めるべきだった。

#### 直し方

`publishNow` で EDITIONS も取得し、`publishSanityCheck` で
EDITION_ID の重複を見る。止めるときは**行番号まで出す。**

```
⛔ 同じ ID の行が重複しています。このまま公開すると必ず失敗します。
  ・EDITIONS "synapse-festival-2026" — 92行目 と 107行目
```

「重複がある」だけでは、シートのどこを直せばいいか分からない。
§9-58 の消し漏れが起きたのも、消す対象を人が探していたため。

#### 止めてはいけないもの

| | 扱い | 理由 |
|---|---|---|
| EDITION_ID の重複 | **止める** | 必ず失敗する |
| LINEUPS の EDITION_ID 重複 | 通す | 出演者ごとに1行なので正しい |
| EDITIONS を取得できなかった | 通す | **この検査のために Publish 自体を止めない** |
| EDITION_ID が空の行 | 通す | 別の検査の担当 |

#### 試験

`scripts/check_cms_publish_guard.mjs`（9件）。実際に起きた重複
（92行目と107行目）をそのまま再現した事例を含む。CI 2本に組み込み済み。
ネガティブコントロール（重複の判定を `>1` から `>2` に変える）で
落ちることを確認済み。

### 9-67. 取得経路によって列が落ち、Publish が空コミットになっていた（2026-08-09）

記事の関連フェスが、**編集画面には出るのに公開に乗らない。**
Publish はエラーも出さず成功し、作られたコミットは**空**だった。

```
5e1e4441 cms: publish data.js  ← 変更ファイル 0件
```

変更が無いので後続も動かない（`LP/data.js` の変更で起動する設計）。
「押したのに何も起きない」という形でしか気づけなかった。
...
**すべてこの日に追加した列**。以前からある `title_en` などは両方に出ていた。

`festPickerSetValue(row.festivalId||'')` は `get_sheet` 由来の行を読むので
画面には出る。`buildArticlesJs(d.ARTICLES)` は `get_all_sheets` 由来を読むので
落ちる。**同じ値を2つの経路で取っていて、片方だけが古い。**
AGENTS.md が繰り返し警告している「同じものを2箇所に持つと片方が古くなる」が、
データの取得経路にも当てはまっていた。

#### なぜ気づけなかったか

**列が落ちてもエラーにならない。**`buildArticlesJs` は
`if(r.festivalId)` で存在するときだけ書くため、無ければ黙って省く。
これは draft の項目を省く正しい動きで、**「無い」と「取れなかった」を
区別できない。**

#### 直し方

Publish と Export は `perSheet:true` で1枚ずつ取る。
公開は頻度が低いので、リクエストが1回から5回に増えても実用上の差は無い。
**速さより、確実に取れることを優先する。**

一括経路そのものは残す（一覧の初回読み込みで使っており、
そこは新しい列に依存しない）。

#### 試験

`scripts/check_cms_fetch_path.mjs`（6件）。
一括経路が列を落とす状況を再現し、

- `perSheet` では一括を呼ばないこと
- `perSheet` なら新しい列が残ること
- `perSheet` 無しは従来どおり一括で取ること（既存動作を壊さない）
- Publish と Export の2経路が確かに `perSheet:true` であること

を見る。CI 2本に組み込み済み。ネガティブコントロールとして
`perSheet:true` を外すと落ちることを確認済み。

#### 残っている疑問

**GAS の `get_all_sheets` がなぜ新しい列を落とすのかは未確認。**
ヘッダー行をキャッシュしている可能性が高いが、GAS のソースを見ていない。
CMS 側は経路を変えて回避したので実害は消えるが、
一括経路を使う他の箇所（一覧の初回読み込み）は同じ問題を抱えたままである。

### 9-68. 列名が1文字違うだけで、値が黙って捨てられる（2026-08-09）

「シートには入っているのに、CMS に反映されない」。

CMS はシートの列名を**完全一致**で読む。`festivalId` を探しているところに
`FestivalId` や `FESTIVAL_ID` があっても、**エラーも警告も出さずに無視する。**

入力した側から見ると、

- シートには確かに値がある
- CMS の画面には出ない
- Publish は成功する（コミットは空になる）
- どこにも「読めませんでした」と出ない

**原因を推測する手掛かりがゼロ。**実際、この件は原因の切り分けだけで
半日以上かかり、途中で「取得経路の違い」（§9-67）という別の要因も見つかったが、
それだけでは説明がつかなかった。

`VIEWS`（シート）と `views`（CMS）は、今も同じ状態で放置されている。
Views は廃止方針なので実害は無いが、**気づかれずに放置されていた**こと自体が
この問題の性質を表している。

#### 直し方: 「惜しい」列を指す

Publish のときに、シートの列名と CMS が読む項目名を突き合わせる。

```
⚠️ 列名が違うため、CMS が読めていない列があります。
  ・"FestivalId" → "festivalId" が正しい綴りです

この列に入力した値は、シートにあってもサイトには出ません。
```

**未知の列を全部警告してはいけない。**メモ用の列などで毎回鳴り、
すぐに無視されるようになる。大小文字・記号・空白を取り除いた形が
既知の項目と一致するものだけを挙げる。

  `FestivalId` / `FESTIVAL_ID` / `festival_id` / `festivalid` → 指摘する
  `編集メモ` / 見覚えのない列                                 → 黙る

#### 試験

`scripts/check_cms_publish_guard.mjs` に6件追加。
「指摘する側」と「鳴らない側」を両方見る。後者が無いと、
うるさくなって機能が死ぬ方向の劣化を検出できない。
ネガティブコントロールとして、正規化を外す（完全一致で照合する）と
落ちることを確認済み。

### 9-69. 取得経路でキーの大文字小文字が変わり、片方を直すと片方が壊れる（2026-08-09）

§9-68 で入れた「列名の綴り違い」警告が、実際に鳴った。

```
⚠️ 列名が違うため、CMS が読めていない列があります。
  ・"readtime"        → "readTime" が正しい綴りです
  ・"metadescription" → "metaDescription" が正しい綴りです
  ・"festivalid"      → "festivalId" が正しい綴りです
```

**3つとも「全部小文字」。**シートの見出しが1つだけ間違うことはあっても、
3つそろって小文字になることはない。**シートではなく、返し方の問題。**

#### 実測

```
get_all_sheets（まとめて）  readTime  ← 見出しの綴りのまま
get_sheet（1枚ずつ）        readtime  ← すべて小文字
```

裏付け: §9-67 の変更前、公開は「まとめて」を使っており、
`237a1fd6` の data.js には `readTime: 2` が入っている。**昔は読めていた。**

#### これが意味すること

§9-67 で公開を「1枚ずつ」に変えたのは、**対症療法として誤りだった。**

```
まとめて取る … festivalId は読めるが、記事一覧・編集画面は小文字のまま
1枚ずつ取る … festivalId は読めるようになるが、readTime が読めなくなる
```

**片方を直すと片方が壊れる。**どちらの経路を選んでも解決しない。
経路を選ぶ問題ではなく、**読む側が完全一致でしか読めない**ことが問題だった。

さらに悪いことに、§9-68 の警告は
**「シートの見出しを直してください」と誤った指示を出していた。**
シートは正しく、直せば逆に壊れる。危うくデータを壊すところだった。

#### 直し方: 取り込んだ時点で名寄せする

`canonicalizeRows()` が、行に**正しい綴りの別名を足す。**
大小文字・記号・空白を落とした形で照合するので、
`readtime` / `READ_TIME` / `Read Time` はすべて `readTime` として読める。

取り込み口5箇所すべてに通した（一覧2 / fetchAllSheets 3）。
これで**以降のコードは1文字も変えずに済む。**

守っていること:

- **元のキーは消さない。**消すと、まだ名前を知らない列を扱う箇所が壊れる。
- **既に正しい綴りのキーがあれば触らない。**上書きすると、
  空の小文字キーが値を潰す。実測で確認した
  （`{readTime:5, readtime:''}` を上書き版に通すと 5 が '' になる）。

#### 警告の側も直した

名寄せで読めているなら、もう「綴りが違う」と言ってはいけない。
**正しい綴りのキーが1行にも無いときだけ**指摘する。

#### 試験

- `check_cms_fetch_path.mjs`（10件）— **両方の経路を実測どおりに再現する。**
  まとめて＝そのまま、1枚ずつ＝小文字。どちらでも `festivalId` と
  `readTime` が読め、data.js まで出ることを見る。
...
- 名寄せを外すと落ちる
- 「空で上書きしない」ガードを外すと値が壊れることを実測

### 9-70. AI が動かない理由が、3秒で消えていた（2026-08-10）

「AI タイトル生成と本文の HTML 翻訳が動かない」という報告。
調べたが、**コード側にはどこにも問題が無かった。**

| 確認したもの | 結果 |
|---|---|
| ボタンと関数の結線（cms.html → cms.js） | 正常 |
| 認証トークンの付与（`withAuthToken_` が POST に注入） | 正常 |
| GAS のエンドポイントが応答するか | 正常 |
| GAS の AI 実装（`check_gas_ai.mjs` 12件） | 全通過 |
| モデル名 `claude-opus-5` | 正しい |

残るのは**実行時にしか分からないこと**——APIキーの設定、デプロイされた版、
モデルへのアクセス権——だが、それは GAS が返す `message` にしか入っていない。

**その message が、3秒で消えるトーストにしか出ていなかった。**

```js
toast('AI failed: '+(d.message||'unknown'), 'error');   // 3秒で消える
```

原因ごとに対処はまったく違う。

```
ANTHROPIC_API_KEY not set   → GAS のスクリプト プロパティにキーを設定
Claude API 401: ...          → キーが無効
Claude API 400: ...          → モデル名・上限トークンの指定ミス
長すぎて途中で切れました        → 本文を分割
```

なのに読む前に消えるので、**利用者は「動かない」としか報告できず、
こちらは「コードは正常です」としか返せない。**先へ進めない。

#### 調査中に踏んだ誤り（記録）

原因を切り分けようとして GAS を curl で叩いたところ
`認証エラー: Invalid auth token` が返り、**一度「トークンを付けずに
送っている」と誤認した。**実際は `window.fetch` のラッパーが
POST の body に `cmsAuth` を注入しており、ブラウザからは正しく送られている。
**トークンを持たない自分の実験が返した正常な拒否**を、バグの証拠と読み違えた。

その前段でも、`curl -X POST -L` がリダイレクト後も POST を維持するため
`405` になり、「GAS が落ちている」と読みかけた。
**外から叩いた結果を、系の異常と取り違えない。**

#### 直し方

AI の失敗を、消えない・選んでコピーできるパネルで出す。
文言をそのまま見せたうえで、代表的な原因には対処を添える。

`toast()` に残っていた AI 系のエラー表示10箇所を `aiFail()` に統一した。

#### 試験

`check_cms_ai_body.mjs` に5件追加（計9件）。
理由の本文がそのまま出ること、キー未設定・400・未知のエラーで
出し分かることを見る。ネガティブコントロールとして、
本文を「エラーが発生しました」に潰すと落ちることを確認済み。

### 9-71. AI が動かない原因は `Claude API 401`（キーが拒否されている）（2026-08-10）

§9-70 でエラー文言を消えない形にしたところ、正体がすぐ出た。

```
Claude API 401: ...
```

401 は「キーは見つかったが、Anthropic 側が受け付けなかった」という意味。
**キー未設定（`not set`）ではない。**設定されているキーが無効。

#### 401 になる原因

| 原因 | |
|---|---|
| キーが失効・削除された | console.anthropic.com で確認 |
| クレジットが尽きた | 同上 |
| **貼り付けたキーの前後に空白や改行が混ざった** | 見た目では気づけない |
| 別サービスのキーが入っている | Google/Gemini のキーなど |
| キーを入れ直したが**再デプロイしていない** | GAS は保存だけでは反映されない |

**キーそのものは触らない。**これは資格情報であり、こちらで設定も確認もしない。

#### コード側で潰せるもの

3つは GAS 側で吸収・明示できる。

1. **前後の空白・改行を落とす。**コピー＆ペーストで紛れ込みやすく、
   見た目では気づけないのに 401 になる。「キーは合っているのに通らない」で
   時間を溶かす典型。
2. **形が違うキーは、API を叩く前に弾く。**`sk-ant-` で始まらなければ
   別サービスのキーである可能性が高い。401 は「無効」としか言わないが、
   これなら**何が違うか**が分かる。
3. **401 のときは確認する場所を文言に含める。**とくに
   「入れ直したら再デプロイが必要」は忘れやすく、
   直したのに直らない状態を生む。

#### 試験

`check_gas_ai.mjs` に6件追加（計18件）。

- 空白・改行付きのキーで通ること
- `sk-ant-` で始まらないキーは**API を呼ばずに**弾くこと
- 401 が元の文言を保ったまま、再デプロイの必要まで伝えること
- 空白だけのキーは「未設定」として扱うこと

ネガティブコントロールとして、空白除去を外すと落ちることを確認済み。

#### 残り

**キーの入れ直しはユーザー作業。**コード側でできることは尽くした。
入れ直しても 401 が続く場合は、キー自体かクレジットの問題。

#### 追記: 「残高はあるのに 401」という混乱（2026-08-10）

コンソールの **Cost $0.40** を見て「残高はあるのに 401 が出る」という
問い合わせがあった。**Cost は使った額であって、残りではない。**
そして 401 は金額とは無関係で、キーそのものが拒否された状態を指す。

```
401  キーの問題       … キーが存在しない・無効
400  クレジットの問題  … "Your credit balance is too low"
```

**原因も対処もまったく違うのに、文言を読まないと区別できない。**
GAS 側で言い切るようにした。

- 400 かつ `credit balance` を含むとき … 「クレジット残高の問題です（キーは有効）」
  と明示し、**Cost ではなく Credits を見ること**を添える
- 401 のとき … 「**残高とは無関係**（残高不足は 400 で返ります）」と明示し、
  迷ったら新しいキーを作るよう促す
- あわせて「**新しいデプロイ**ではなく既存のデプロイを編集」も文言に入れた
  （新規デプロイは URL が変わり、CMS 全体が止まる）

無関係な 400（`max_tokens` 超過など）でクレジットの話をしないことも試験する。
**誤った案内は、案内が無いより悪い。**

### 9-72. 翻訳が2つ定義されていて、古いほうが動いていた（2026-08-10）

`callClaude_` の貼り替えに失敗して `コード.gs` が構文エラーになった。
ブラウザ操作で GAS を直接見に行ったところ、**別の問題が見つかった。**

```
aiTranslateV2_  1210行目  新しい版（callClaude_ を呼ぶ / Opus 5）
aiTranslateV2_  1541行目  古い版（自分で UrlFetchApp を叩く / Sonnet 5）
```

**JavaScript は後の宣言が勝つ。**つまり **1541行目の古い版が動いていた。**

2026-08-07 に「Claude Opus 5 に統一した」と記録しているが、
**翻訳については効いていなかった。**新しい版を貼り足したものの、
古い版が下に残っていて上書きしていた。

要約（`aiSummarize`）は1つしか無く `callClaude_` を通るので、
**タイトル生成だけが新しい版で、翻訳は古い版**という食い違った状態だった。

#### なぜ気づけなかったか

- **重複した関数宣言は構文エラーにならない。**静かに後勝ちするだけ。
- `check_gas_ai.mjs` は**リポジトリのファイル**を検証する。
  GAS に実際に入っている中身は見ていない。**手で貼る運用である以上、
  リポジトリと実物が一致している保証はどこにも無い。**
- 症状（401）が両方の版で同じだったため、区別がつかなかった。

#### 直したこと

1541〜1574行目（見出しコメント＋古い関数＋余分な空行）の35行を削除。
適用前に検証し、適用後にも確認した。

```
構文            ok
aiTranslateV2_  2 → 1
関数の総数       54 → 53（消えたのは1つだけ）
残った翻訳       callClaude_ 経由
```

#### 調査の記録

エディタの中身は `monaco.editor.getModels()` から読める。
`new Function(src)` に通せば**構文の正否をその場で判定できる**ので、
壊れた箇所を推測せずに特定できた。

**コードの中身をそのまま画面に出そうとすると、APIキーらしき文字列を
含む行が遮断された。**行番号・関数名・真偽値だけを返す形に変えて回避した。
資格情報を含みうるファイルを覗くときは、最初からそうすべきだった。

#### 残り

**401 の原因はこれではない。**新旧どちらも同じ `ANTHROPIC_API_KEY` を
読んでおり、キー自体が無効。キーの入れ直しはユーザー作業。

**この修正はまだデプロイしていない**（保存のみ）。

### 9-73. `content[0]` だけを見ていて、本文があるのに「空」と判定していた（2026-08-10）

APIキーを入れ直したところ **401 は解消**し、次に出たのがこれ。

```
Claude API: 応答が空でした
```

HTTP は 200。本文も返っている。**取り出し方が間違っていた。**

```js
var text = (body.content && body.content[0] && body.content[0].text) || '';
```

応答の `content` は**ブロックの配列**で、`content[0]` が本文とは限らない。
モデルによっては thinking ブロックが先頭に来る。先頭だけを見ると、
本文が2番目以降にあっても空と判定してしまう。

**私が書いたコードの誤り。**§9-70 でエラー表示を直していなければ、
「AI が動かない」で止まったままだった。

#### 直したこと

1. **`type === 'text'` のブロックを全部つないで返す。**
   `type` を持たない古い形式にも備える。
2. **打ち切り判定を、空判定より先に行う。**
   thinking が上限を食い切ると本文が空になる。そこで「応答が空」と
   報告すると、**本当の原因（上限）に辿り着けない。**順序に意味がある。
3. **空のときは「何が返ってきたか」を添える。**

```
Claude API: 本文が取り出せませんでした（ブロック thinking / stop_reason end_turn）
```

これが無いと、応答の構成が変わるたびに同じ場所で詰まる。

#### 試験

`check_gas_ai.mjs` に5件追加（計28件）。

- 先頭が thinking でも本文を取り出す
- text ブロックが複数なら全部つなぐ
- 本当に空ならブロック構成を添える
- 打ち切りを「空」より優先して報告する
- `type` の無い応答でも取り出せる

ネガティブコントロールとして、`content[0]` だけを見る元の実装に戻すと
落ちることを確認済み。

#### 適用方法（記録）

手で貼り替えて構文を壊した反省から、**ブラウザ操作で Monaco を直接編集**した。

1. `monaco.editor.getModels()` で対象を取り、置換範囲を**関数の構造から**求める
2. 置換後の文字列を作り、**適用前に** `new Function()` で構文検証し、
   関数の総数が変わらないことを確認する
3. 検証を通ってから `pushEditOperations` で適用し、適用後にも再確認する

**行番号を目視で数えて選択する作業を無くす。**それが構文エラーの原因だった。

#### 追記と訂正: 真犯人は `content[0]` のほうだった（2026-08-10）

一度「真犯人は `MAX_TOKENS_SUMMARY = 1000`」と書いたが、**これは誤りだった。**

実測で確定した経緯:

```
バージョン57 (1:10)  重複削除 + キーの空白除去    翻訳○ / タイトル×（応答が空）
バージョン58 (1:16)  + content[0] の取り出し修正   翻訳○ / タイトル○
                     ※ MAX_TOKENS は 1000 のまま
```

**タイトル生成を直したのは content ブロックの取り出し修正**であって、
上限の値ではない。バージョン58 に上限の変更は入っていない。

なぜ誤ったか。「翻訳（16000）は動き、タイトル（1000）は動かない。
差はこの数字だけ」という推論を、**確かめる前に結論として書いた。**
実際には、その2つは `content` の構成も違っていた可能性が高い
（翻訳は先頭が text、要約は先頭が thinking だった）。

**症状の差を説明できる仮説を1つ見つけただけで、原因と決めつけた。**
検証手段（デプロイのバージョンと、そこに何が入っているか）は
最初から手元にあった。

`MAX_TOKENS_SUMMARY` を 4000 にした変更自体は残す。
上限は枠であって課金は実際の出力分なので、絞る理由が無い。
ただしこれは**予防であって、今回の修正ではない。**

### 9-74. リポジトリの .gs と GAS の実物がずれても、誰も気づけなかった（2026-08-10）

§9-72 で「翻訳が二重定義され、古い版が動いていた」ことが分かった。
**記録と実物が3日間食い違っていた**のに、検出する手立てが無かった。

`check_gas_ai.mjs` は**リポジトリのファイル**を検証する。
GAS に何が入っているかは見ていない。**手で貼る運用である以上、
両者が一致している保証はどこにも無かった。**

#### やったこと

**1. 他のファイルにも重複が無いかを実測**

5ファイル・73関数を走査。**重複は他に1件も無かった。**

**2. リポジトリと実物を突き合わせる仕組みを作った**

```
scripts/gas-update/snapshot.js     GAS 側で実行し、指紋を出す
scripts/gas-update/live-snapshot.json  その指紋を置く
scripts/check_gas_sync.mjs         リポジトリの .gs と突き合わせる（CI 2本）
```

**コードそのものはリポジトリへ持ってこない。**GAS 側にキーが混ざったとき
一緒に取り込んでしまう。**指紋なら、中身を持ち出さずに一致だけ確かめられる。**

指紋はコメントと空白を無視する。説明文を直しただけでは鳴らない。

**3. 実物をリポジトリ版に揃えた**

作った検査が**さっそく食い違いを検出した。**GAS の `callClaude_` には
キーの空白除去・`sk-ant-` の形式確認・401/400 の言い分けが**入っていなかった**
（正規化長 1608 に対しリポジトリは 2452）。リポジトリ版で置き換え、
3関数・3定数すべての指紋が一致することを確認した。

#### 途中で犯した誤り（記録）

「空白除去が入っているか」をファイル全体への文字列検索で確かめ、
**`true` と報告した。**実際には別の関数（`buildRowFromHeaders`）にある
同じ記述 `.replace(/\s+/g, '')` を拾っていた。

**存在確認は、対象の範囲を絞ってから行う。**ファイル全体に対する
「含むか」は、同じ書き方が他にある限り何の保証にもならない。

#### 試験

ネガティブコントロール3種を確認済み。

- リポジトリだけ直して貼っていない → **落ちる**
- コメントだけ直した → **鳴らない**（うるさくして無視される方向の劣化を防ぐ）
- 定数だけ変えた → **落ちる**

#### 運用

GAS に貼って**再デプロイした後**に `snapshot.js` を実行し、
`live-snapshot.json` を更新する。手順はそのファイルの先頭に書いた。

### 9-75. 一時的な 404 で画像同期が落ちていた（2026-08-12）

```
--- Syncing articles ---
Failed to fetch articles: HTTP Error 404: Not Found
```

GAS は一時的に 404 や 5xx を返すことがある。**実体は何も壊れていないのに
同期全体が赤くなる。**1回の失敗で諦める作りだったのが原因。

直前の実行（05:48）は成功しており、次も何もせず成功する類の失敗。
**「たまに赤くなる」は、赤の意味を薄める。**本当に壊れたときに
「またいつものやつだろう」と流される。

#### 直したこと

間隔を空けて3回試す（10秒・20秒）。それでも駄目なら本当の失敗として扱う。
あわせて `urlopen` に `timeout=60` を付けた。**応答が返らないときに
永久に待つ**のを防ぐ（§9-65 の「順番待ちのまま固まる」と同じ性質の穴）。

#### 試験

`scripts/check_sync_retry.py`。ワークフローに埋め込まれた Python から
再試行部分を取り出して確かめる。**再試行は普段は動かない道**なので、
壊れていても気づけない。

- 構文が正しい / `time` を import している / `timeout` がある
- 1回失敗しても2回目で成功すれば通る
- 1回目で成功したら待たない（無駄な待ちを入れない）
- 3回とも失敗したら失敗として扱う
- 最後の試行のあとには待たない

ネガティブコントロール2種（`import time` を消す / `timeout` を消す）で
落ちることを確認済み。

### 9-76. クラブ地図が東京しか出していなかった（2026-08-12）

`map.html` は `v.city === 'TOKYO'` で絞っていた。
**大阪6件・京都1件が地図に1つも出ていなかった。**

一覧は `[...new Set(VENUES.map(v => v.city))]` で都市を自動生成しており、
新しい都市を足せば勝手に追随する。**地図だけが取り残されていた。**
福岡・札幌を足しても同じことが起きる状態だった（§9-60 の会場調査で
「急がない」と判断していたもの）。

#### 絞り込みを外すだけでは足りない

3つ直して初めて成立する。**どれか1つ欠けると、静かに元通りになる。**

| | 直さないと |
|---|---|
| `minZoom: 11` → `5` | **日本全体が画面に入らない。**引けないので東京以外のピンに辿り着けない |
| 固定の中心・ズーム → 会場に合わせて寄せる | 増えた都市が画面外に出る |
| 東京の地名ラベル（SHIBUYA 等） | 引いた状態で地名が散らばって邪魔になる |

#### 途中で踏んだ2つの罠

**1. 高さ 0 の `fitBounds` は黙って無視される**

`#map` は `position:absolute` で、読み込み直後は高さが 0 のことがある。
その状態で `fitBounds` を呼んでも**何も起きず、エラーも出ない。**
実際、同じコードでズームが 9 になったり 13 になったりした。
`whenReady` と `invalidateSize` を挟んで、大きさを確定させてから呼ぶ。

**2. `const` の宣言順で、コールバックが静かに死ぬ**

`applyAreaLabelVisibility()` を `whenReady` の中から呼んでいたが、
そこで使う `const AREA_LABEL_MIN_ZOOM` の宣言が**後ろにあった。**
関数宣言は巻き上がるが `const` は巻き上がらない（未初期化のまま参照すると
ReferenceError）。**コールバックの中なので例外は画面に出ず、
「ズームは合っているのにラベルだけ消えない」という形で現れた。**

どちらも「1回動かして目で見る」では気づけない。

#### 恒久チェック

`scripts/check_map_nationwide.mjs`。実際に描画して、

- data.js にある全都市のピンが出ること
- 会場の件数だけピンがあること
- `minZoom` が日本全体まで引ける値であること
- 引いた状態で東京の地名ラベルが隠れること
- **5回読み込んで結果がぶれないこと**（初期化のタイミング依存を検出する）

を PC 幅とモバイル幅の両方で見る。CI 2本に組み込み済み。

ネガティブコントロール3種で落ちることを確認:
東京限定に戻す / `minZoom` を 11 に戻す / `const` を後ろへ移す。

**1回目のネガティブコントロールは不正確だった**（`zoomend` だけ外したが
`moveend` が残っていて実際には動いていた）。**壊し方が本物と違うと、
検査が効いていることの証明にならない。**実際の不具合を正確に再現し直した。

#### 表題

「TOKYO CLUB MAP」は実態と合わなくなったので「CLUB MAP」にした。
`<title>` と meta description も同様。

### 9-77. 「テストしてから本番へ出す」を、約束ではなくコマンドにした（2026-08-12）

ユーザーが定めた**絶対のルール**。

> 毎回いきなりデプロイするのではなく、必ずテストをしてから本番環境に出す。

#### なぜ守れていなかったか

このリポジトリは **main へ push した時点で本番へ出る**。
`deploy-pages.yml` が自動で走るため、**push は「保存」ではなく「公開」**。

にもかかわらず、検査は CI 側にしか無かった。
「あとで CI が見てくれる」は成立しない。**CI が落ちたときには、
もう本番に出ている。**

そして——**検査そのものは23本あった。まとめて回す手段が無かっただけ。**
1本ずつ思い出して叩く運用は続かない。実際、2026-08-07〜12 の2週間で
本番が壊れた／止まった事故が続いた（§9-58 / §9-66 / §9-72 / §9-76）。

#### やったこと

`scripts/preflight.sh` を作り、25本を1コマンドで通せるようにした。

```
bash scripts/preflight.sh          # 全部（ブラウザ検査を含む）
bash scripts/preflight.sh --fast   # ブラウザ検査を省略（下書き確認用）
```

分類して出す（生成物 / キャッシュ / CMS / GAS / CI 自身 / 実際に描画するもの）。
落ちたものは名前を並べ、**「本番へ出さないこと」と明示して終わる。**

**`--fast` で通っても「成功」と言わない。**警告を出し、
省略なしで通すよう促す。実際に描画しないと分からない不具合が
繰り返し起きているため（入力欄の重なり §9-63 / 地図の初期化 §9-76）。

さらに、**自動化できていない確認**（CMS の実操作、GAS の再デプロイ）を
最後に必ず表示する。**黙って省かせない。**

#### 置き場所

AGENTS.md の**冒頭**に置いた。データ構造や文章の書き方より前。
毎回読まれる位置でないと、ルールは守られない。

#### この記録自体の扱い

本コミットも、push 前に `preflight.sh` を全件通してから出した（25/25 成功）。

### 9-78. 会場一覧の地図が、5都市べた書きだった（2026-08-12）

`venues.html` は TOKYO / OSAKA / NAGOYA / KYOTO / HAKUBA の**5都市を
べた書き**していた。ほぼ同じコードが5つ並び（約280行）、
HTML の入れ物も CSS の指定も5つ分あった。

**対応表に無い都市は、MAP を押しても何も起きない。**

```js
const cfg = ALL_MAPS_CFG.find(m => m.city === city);   // undefined
if (cfg) { ... }                                       // 何もせず素通り
btn.classList.add('active');                           // ボタンだけ光る
```

エラーも警告も出ない。**ボタンは押せた顔をして、地図は出ない。**

地域ナビ自体は `[...new Set(VENUES.map(v => v.city))]` で自動追随する。
つまり**一覧には出るのに、地図には出ない**という食い違いが起きる。
NAGOYA と HAKUBA は逆に、地図はあるのにデータが0件だった。
...
**直す前に、壊れていることを実測で押さえる。**そうでないと、
直した後の「動いた」が何の証明にもならない。

#### 直し方: 都市名をコードから消す

`VENUES` にある都市の分だけ、入れ物も地図も作る。

- HTML の入れ物5つ → `#city-maps` 1つ（`buildCityMapSections` が生成）
- CSS の id 列挙 → `.city-map-wrap` / `.city-map-filters` / `.city-map`
- JS の5複製 → `createCityMap(city)` 1つ（状態は関数の中に閉じる）
- `ALL_MAPS_CFG` の手書き配列 → `citiesWithMaps()` から生成

**268行 → 147行。**

**中心座標も持たない。**都市ごとの座標表を持つと、都市が増えるたびに
書き足すことになり、書き忘れれば的外れな場所が映る。
その都市の会場から `fitBounds` で求める。§9-76 と同じ考え方。

#### 検査が特定の都市に依存していた

`check_hub_pages.py` のタイル失敗フォールバック検査が、
東京の `venue-map-wrap` を**id で直接指していた。**改修で id が変わり落ちた。

**preflight がこれを捕まえた。**push する前に止まった。ルール（§9-77）を
入れた初日に、実際に事故を1件防いだ。

検査側も `.city-map-wrap` をクラスで取る形に直した。
**検査が特定の都市に依存していると、都市を増やすたびに検査の方が壊れる。**

#### ネガティブコントロールで2回失敗した（記録）

1. 「絞り込みの生成を消す」つもりの置換が、実際には**文字列が一致せず
   何も変えていなかった。**それに気づかず「検査が形骸化している」と
   報告しかけた。
2. インデントが4スペースなのに6スペースで書いていた。

**壊したつもりで壊れていないと、「検査が甘い」という誤った結論に至る。**
置換後に必ず件数を数え、変わったことを確認してから判定する。

#### 恒久チェック

`scripts/check_venue_maps.mjs`。実際に各都市の MAP を押して、
入れ物が出るか・ピン数が会場数と一致するか・絞り込みが効くか・
同時に開くのは1つだけかを見る。CI 2本と preflight に組み込み済み。

ネガティブコントロール3種（都市の抽出を東京限定に戻す／入れ物の生成を
止める／絞り込みの生成を消す）で落ちることを確認済み。

### 9-79. 出演者の81%が構造化データから消えていた／診断の優先1〜6を実施（2026-08-13）

診断（reports/diagnosis-2026-08-13.md）の優先1〜6をまとめて実施した。
その過程で、**診断自体の誤りが2つ**見つかった（診断書末尾に訂正を記載）。

#### 本命: performer がほぼ出ていなかった本当の理由

「生成コードに performer が無い」と診断したが、**コードは8/1から存在していた。**
実際の原因は3つのデータ経路の断線:

| 見えていた症状 | 本当の原因 | 対処 |
|---|---|---|
| performer が出ない | LINEUPS 621行中 **501行が ARTIST_ID 無し**（名前だけ）。リンク必須の lineupEntity が全部捨てていた | 名前だけでも `MusicGroup` として出す（lineupPerformerLd） |
| eventStatus が出ない | EDITIONS.STATUS が**規約外 `published` 36件・空欄55件**。正規値のみ変換していた | 既定を EventScheduled に（cancelled だけは明示値が必要なので既定にしない） |
| offers が出ない | TICKETURL があるのは 104回中 20件 | コードは正常。**データ入力の課題として報告** |

修正後: 出演データのあるフェス**31件すべて**に performer が点灯
（LOA は13名）。親 Festival の performer は開催回をまたぐ重複を名寄せ。

**規約外の STATUS=published 36件は直していない**（AGENTS.md: 勝手に直さず報告）。
シート側で announced 等への修正が必要。→ ユーザーへ報告済み。

#### 実施した残りの項目

1. **robots.txt の `/map.html` 封鎖を解除**（ユーザー承認済み）。
   全国対応した地図が検索から見えなかった。cms.html の封鎖は維持。
2. **llms.txt / events.json をビルド生成**（buildAiSurface）。
   今後の開催回17件を日付・場所・座標・チケット付きで機械可読に。
   **タイムスタンプを埋めない**（毎日の再生成コミットで無意味な差分が
   出るのを防ぐ。§9-58 の ?v べた書きと同型の罠）。
3. **タップ領域**: `.vm-pill` / `.filter-pill` に touch-action: manipulation、
   タップ端末（pointer: coarse）でのみ min-height 44px。PC の見た目は不変。
4. **Lighthouse にカテゴリ別の床**を追加。実測（性能67 / A11y89 / BP96 /
   SEO92）の**すこし下**に張った（性能0.60 / A11y0.85 / BP0.90 / SEO0.90）。
   理想値（0.85等）を置くと初日から赤くなり、赤が無視される。
5. **エリア名ラベルに aria-hidden**（コントラスト1.65:1 の装飾。
   読ませる意図が無いことを支援技術に宣言）。

#### 恒久チェック scripts/check_jsonld.mjs（CI 2本 + preflight）

**構造化データは目に見えない。**壊れても消えても画面は1ピクセルも
変わらず、気づくのは流入が減ってから。実際、performer の消失は
**6週間誰も気づかなかった。**

- 449ページ・461ブロックの ld+json がパースできること
- **出演データのあるフェス全件に performer が出ていること**
  （lineups.json と突き合わせる。これが本命）
- 開催回すべてに eventStatus / events.json の必須項目 / llms.txt の案内
- robots.txt が map.html を隠していないこと

ネガティブコントロール2種（名前だけ対応を外す＝元のバグ再現／
robots の封鎖を戻す）で落ちることを確認。**置換後に件数を出力して
「本当に壊した」ことを確かめてから判定した**（§9-78 の反省の適用）。

#### 診断の教訓

ページに出ていない原因は「コードが無い」「データが無い」「解決が切れている」の
3通りある。**生成コードを読まずに「コードに無い」と断定した。**
しきい値も同様（CLS の床は存在した）。断定の前に、必ず実装を読む。

### 9-80. Publish が2回続けて届かず、原因が3秒で消えていた（2026-08-13）

シートにアーティストの紹介文を11件入れたのに、Publish Now を2回押しても
**リポジトリに `cms: publish data.js` が1件も来なかった。**

```
スプレッドシート  BIO 11件 / INSTAGRAM 10件   ← 入力は保存されている
公開中の data.js  BIO  4件                    ← 変わらない
```

#### 原因1: GAS へ5本同時に投げていた（私の作り込み）

§9-67 で「公開は1枚ずつ取る」に変えたとき、`Promise.all` で書いた。

```js
return Promise.all(missing.map(s => fetch(GAS_URL + '?action=get_sheet&sheet=' + s) ...
```

**GAS は同一デプロイへの同時実行に制限がある。**5本のうち1本でも失敗すると
`throw` して Publish 全体が止まる。batch 1本だったものを5本並列に変えた時点で、
この失敗の目を自分で作っていた。

`reduce` で**逐次**に変更した。公開は頻度が低いので、速さより確実さを取る。

#### 原因2: 失敗の理由が3秒で消えていた

`toast('Publish error: ' + e.message, 'error')` は3秒で消える。
**利用者は「押したのに何も起きない」としか観測できない。**
§9-70 で AI 機能に対して直したのと**まったく同じ問題**が、Publish に残っていた。

`aiFail()` を Publish からも呼ぶようにし、対処の案内を拡張した
（シート取得の失敗 → 再試行を促す / 認証エラー → 入り直しを促す）。

**一度直した問題は、同じ形で別の場所に残っている。**
横展開を確認しなかったのが漏れの原因。

#### 恒久チェック

`check_cms_fetch_path.mjs` に「**同時に走るのは常に1本だけ**」を追加。
fetch を遅延させて実際の重なりを数える。
ネガティブコントロールとして `Promise.all` に戻すと
「最大5本が同時に走った」で落ちることを確認済み。

---

### 9-80b. 同日に起きた他の2件

#### Lighthouse: 「測れなかった」を「基準割れ」と判定していた

トップページで LCP が記録されず性能スコアが `null` になり、
§9-79 で入れた `minScore` が CI を落とした。

```
/          性能 null（LCP 記録なし）  A11y 95 / BP 96 / SEO 100 は正常
他7ページ   性能 70〜91               すべて正常
```

ページが壊れたのではなく、**混雑した CI 実行環境で計測が取れなかっただけ。**
性能は共有ランナーから実サイトを測る以上、本質的に揺れる。

**揺れる指標でビルドを止めない。**性能を `warn` に下げ、実行環境に
左右されない a11y / best-practices / seo を `error` のまま残した。
床を下げて通すのではなく、**止める条件そのものを選び直す**のが正しい。

#### 画像同期: push の衝突で落ちていた

画像を作り終えた直後の `git push` が、別の push と重なって弾かれた。
実体は何も壊れていない（同じ画像は先行の実行が既に送っていた）。
`git pull --rebase` を挟んで3回まで再試行するようにした。
生成物の追加なので rebase で衝突しない。

#### 画像と Publish の関係（記録）

同期された画像は **サーバーに置かれるだけで、ページからは参照されない。**
どのアーティストがどの画像を使うかは data.js に書かれており、
それを作るのは Publish。**Publish が届かない限り画像も表示されない。**
...
この関係は分かりにくく、「画像を同期したのに出ない」と受け取られやすい。

---

### 9-82. 英語版の ABOUT が、CSS の無い素の HTML で公開されていた（2026-08-14）

利用者から「英語のページがこれになってる」とスクリーンショットで報告があった。
文字は黒、背景は白、Instagram のアイコンだけが画面いっぱいに引き伸ばされていた。

本番 `https://techno-japan.media/en/about.html` を実ブラウザで測ったところ:

```
背景色              rgba(0, 0, 0, 0)     ← 透明。CSS が1つも効いていない
文字色              rgb(0, 0, 0)         ← ブラウザ既定の黒
スタイルシート      /en/common.css = 0   ← 404。ルールが0本
最初のSVGの大きさ   1637 × 1637 px       ← 幅指定が無く原寸で出ていた
```
...
```html
<!-- LP/about.html -->
<link rel="stylesheet" href="common.css?v=6">     ← 相対パス
```

ルート直下の `/about.html` から見れば `/common.css` に解決するので、
**日本語版は完全に正常に見える。** ところが同じファイルが `/en/about.html`
として置かれると、ブラウザは `/en/common.css` を探す。そこには何も無い。
CSS 5本すべてが同じ理由で 404 になり、素の HTML だけが残った。

#### なぜ気づけなかったか

- **日本語で見て正しいことが、英語で正しいことの根拠にならなかった。**
  原本と複製で、同じ1行の意味が変わる種類の壊れ方だった。
- **HTTP は 200 を返していた。** 落ちていたのは中の CSS / JS で、
  ページ自体は正常に配信されている。リンク切れ検査は通ってしまう。
- `about.html` だけが相対パスだった。他のページは全部 `/common.css` で、
  ABOUT だけが例外。1枚だけなので目視の比較にも残らなかった。

#### さらに、直しても直らなかった

`LP/about.html` を絶対パスに直して再生成したが、結果は変わらなかった。

```
$ node scripts/build-detail-pages.mjs
EN hubs written: none (up to date)
$ grep common.css LP/en/about.html
href="common.css?v=6"          ← 相対のまま
```

`build-detail-pages.mjs` が英語版を生成しているのは **5ハブだけ**だった。

```js
const HUBS = ['index.html', 'festivals.html', 'artists.html', 'venues.html', 'news.html'];
```

`about.html` と `submit.html` はこの一覧に無い。**過去に一度手でコピーされ、
以後どこからも再生成されない置き物**になっていた。だから原本をいくら直しても
複製には届かない。EN 側のファイルを直接直す必要があった。

生成される5ハブは `enHubFromJa` が生成時に絶対パスへ書き換えるため、
最初から無事だった。**生成経路に乗っているものだけが守られていた。**

#### 直したこと

- `LP/about.html` の5本を絶対パスへ（原本）
- `LP/en/about.html` の5本を絶対パスへ（複製。ここが本番に出ている実体）
- `LP/about.html` に、なぜ絶対でなければならないかをコメントで残した

#### 再発を止める仕組み

`scripts/check_asset_paths.py` を追加し、preflight の「キャッシュと配信」に入れた。
2つを見る。

1. **参照先が実在するか** — 全473ページの `href` / `src` が指す .css / .js を
   そのページの位置から解決し、ファイルの有無を確かめる。
   `en/about.html` の `common.css` は `LP/en/common.css` に解決して不在 → 検知。
2. **手でコピーされる原本の相対パス** — ルート直下のうち `LP/en/` に同名の
   複製があり、かつ生成5ハブでないページ（= about / submit）が相対パスを
   持っていたら止める。検査1は原本側を通してしまうため、**原本の側でも**禁じる。

ルート直下の他ページが `common.css` と書くのはサイトの既定で、
`/common.css` に正しく解決する。これは問題にしない。

ネガティブコントロールは3経路とも検知した。

```
NC: LP/en/about.html を相対に戻す  → ✅ 「LP/en/common.css が無い」
NC: LP/about.html を相対に戻す     → ✅ 「en/about.html 側で 404」
NC: LP/submit.html を相対に戻す    → ✅ 「en/submit.html 側で 404」
```

#### ネガティブコントロールの破壊が、一度ずれていた

最初の破壊コードは「置換1件」と報告したのに検査が反応しなかった。
検査の不備を疑ったが、実際には**破壊側が別の場所を壊していた**。

```
正規表現       (href|src)="/([a-z][a-z0-9_.-]*\.(?:css|js))
実際の一致箇所  href="/manifest.js       ← manifest.json の .js に当たっていた
```

`.json` の中の `.js` を掴んでいた。置換件数が1でも、
**狙った場所が変わったとは限らない。** §9-70 で「置換件数を数えてから判断する」
と決めたが、件数だけでは足りず、**何に当たったかまで印字する**必要があった。
破壊対象を出力させたら即座に分かった。

#### 教訓

- **JA が正しいことは EN が正しいことの根拠にならない。**
  原本と複製で解決結果が変わる書き方は、片方でしか壊れない。
- **生成経路に乗っていないファイルは、直しても反映されない。**
  「再生成した」は「反映された」ではない。生成後に実物を grep して確かめる。
- 資産は絶対パスで書く。ファイルがどこへ置かれても壊れないため。
- ネガティブコントロールは、件数ではなく**一致した文字列を印字**して確かめる。

---

### 9-83. CMS の Image Position が、アーティストと会場の詳細ページに届いていなかった（2026-08-14）

利用者から「アーティストの CMS の Image Position は機能してる？」と質問があり、
入力から表示までを一本ずつ追って確かめた。**半分だけ機能していた。**
...
WATA IGARASHI は CMS で `center top` を指定済み。詳細ページを実ブラウザで測ると:

```
CMS の指定       center top
実際の描画       50% 50%        ← 中央。指定が無視されている
枠 860×573 / 原画 1440×1440
縦の33%が切られ、上から17%（＝頭）が消えていた
```

CMS の入力欄には「頭が切れるときは top を選ぶ」と書いてある。
**その症状を直すための機能が、直すべき場所で効いていなかった。**

#### 原因

`build-detail-pages.mjs` で、フェスの hero だけが `object-position` を出していた。

```js
// 1148行 フェス（出していた）
<img ... style="object-position:${esc(f.imagePosition || 'center')}">

// 1346行 アーティスト（出していなかった）
<img ${dimensionAttrs(a.image)} src="..." alt="${esc(name)}">

// 1416行 会場（出していなかった）
<img ${dimensionAttrs(v.image)} src="..." alt="${esc(name)}">
```

写真は `.detail-hero img { object-fit: cover }` で枠に合わせて切り抜かれる。
位置指定が無ければ必ず中央基準になる。
...
```
CMS に入力欄がある          ✅  cms.html 279行
入力が保存される            ✅  cms.js  3994行
data.js に書き出される       ✅  cms.js  6363行
一覧カードに反映される       ✅  artists.html 724行
詳細ページに反映される       ❌  ← ここだけ
```

途中がすべて正しく動くので、CMS を触っている限り「保存できた」と見える。
**画面を実際に見る以外に気づく方法が無かった。**
しかも `center` 以外を指定していたのはアーティスト96名中1名だけで、
残り95名は指定が無く、無くても中央で正しいため差が出なかった。

#### 直したこと

同じ式が4箇所に散っていたため（フェス hero / 回遊カード2種 / 新規2種）、
**1本の関数に寄せてから**全箇所で使うようにした。
...
散らしたままだと、次に誰かが規則を変えたとき片方だけ古くなる。
`localizedValue` が2箇所にある件（AGENTS.md）と同じ罠を作らないため。

#### 修正後の実測

```
WATA IGARASHI  指定 center top → 実際 50% 0%   ✅
  縦の33%は依然として切られるが、切られるのは**すべて下側**。上から0%
Acid Pauli     指定なし        → 実際 50% 50%  ✅
UNIT（会場）    指定 center top → 実際 50% 0%   ✅
英語版も同じ                                    ✅
```

#### 再発を止める仕組み

`scripts/check_image_position.mjs` を追加し、preflight の「生成物とデータ」に入れた。
data.js の全項目について、生成された詳細ページの hero に同じ `object-position` が
入っているかを JA / EN 両方で照合する（150ページ）。
...
修正後の計測で「上から17%切れている」と出て、直っていないように見えた。
だが原因は**計測式**だった。

```js
var f = ... : (parseFloat(py)/100 || 0.5);
//  py = "0%" → parseFloat → 0 → **0 は偽値なので 0.5 に落ちる**
```

`object-position: 50% 0%`（＝上端合わせ）を、中央として計算していた。
実際の切り落としは下側100%で、上は0%。

`||` によるフォールバックは、**0 と空文字を「未設定」と同じ扱いにする。**
§9-69 で「大文字小文字で値が消える」を踏んだのと同じ形の罠が、
今度は検証コード側に出た。**測る側を疑う手順が要る。**

#### 教訓

- **入力欄がある・保存される・データに載る、は「効いている」ではない。**
  経路の最後の1行が欠けても、途中はすべて正常に見える。
- 同じ規則を複数箇所に書いたら、いつか片方だけ古くなる。先に1本化する。
- 期待と違う計測値が出たら、**対象ではなく計測式を先に疑う**。
  `||` は 0 を握りつぶす。

---

### 9-84. CMS のプレビューが、実ページと違う見え方をしていた（2026-08-14）

§9-83 で Image Position を詳細ページに効かせた直後、
「プレビューで確認できるようになってる？」と聞かれて調べた。
**確認画面のほうが実ページと食い違っていた。**

CMS には位置を確かめる場所が2つある。両方に問題があった。

#### 1. 入力欄の下の小さなプレビュー（img-pos-preview）

位置は反映していた（`background-position` を JS が即時に更新する）。
だが**枠の形がアーティストだけ違った。**

| | プレビューの枠 | 実ページの枠 |
|---|---|---|
| アーティスト | 16:9 (1.778) | **3:2 (1.500)** |
| 会場 | 16:9 | 16:9 ✅ |
| フェス | 16:10 | 16:10 ✅ |

正方形の写真（WATA IGARASHI は 1440×1440）で計算すると:

```
プレビュー 16:9   写真の縦 56% が見える（44% が切れる）
実ページ   3:2    写真の縦 67% が見える（33% が切れる）
```

**プレビューは実際より11%多く切って見せていた。**
プレビューで「切れている」と思って調整しても、本番では別の切れ方になる。

#### 2. 「👁 Preview」の全画面プレビュー（pv-hero-image）

こちらは**もっと悪く、`object-position` を一切出していなかった。**
...
Image Position を `top` にしても、全画面プレビューは中央のまま。
枠もアーティストは `aspect-ratio:1/1`（正方形）で、実ページの 3:2 と別物だった。

**位置を合わせるための機能なのに、確認画面がその調整を見せていなかった。**

#### なぜ気づけなかったか

§9-83 と同じ構図。**経路の一部だけが欠けていた。**

```
CMS に入力欄がある             ✅
小プレビューが位置に反応する    ✅  ← ここが動くので「効いている」と見える
小プレビューの枠が実ページと同じ ❌
全画面プレビューが位置を反映     ❌
詳細ページに届く                ✅（§9-83 で修正済み）
```

小プレビューが指定どおりに動くため、**枠の形まで疑う理由が無かった。**
そして枠のずれは、両方を並べて測らない限り分からない。

#### 直したこと

- 小プレビュー: アーティストを `aspect-ratio:16/9` → `3/2` に
- 全画面プレビュー: `pvHeroStyle(prefix)` を追加し、3種すべての hero に適用。
  枠の比率と `--pv-pos`（CSS 変数）を出す
- `cms.css`: `.pv-hero-image img` に `object-position:var(--pv-pos,center)` を追加
- 3箇所すべてに「実ページの枠と揃える」根拠をコメントで残した
...
`scripts/check_cms_preview_frame.mjs` を追加し、preflight の「CMS」に入れた。
**枠の値をべた書きせず、`detail.css` の実物から読んで突き合わせる。**
detail.css 側を変えたら検査が自動的に追随し、CMS 側だけ古いと落ちる。
...
#### CMS を丸ごと開く検証は、2回とも5分でタイムアウトした

headless Chrome で `cms.html` を開いて計測しようとしたが、
GAS への通信待ちで `--dump-dom` が返らず、2回とも時間切れになった。
`--virtual-time-budget` は保留中のネットワークを打ち切らない。

**実物の cms.js から対象の関数だけを正規表現で切り出し**、
同じ CSS を読み込んだ最小ページで動かして計測した。
コピーではなく実物を切り出すので、本体と食い違う心配は無い。
認証が要る画面を測るときの型として残す。

#### 教訓

- **プレビューは「動いている」だけでは足りない。実物と同じ形かまで見る。**
  反応することと、正しく見せることは別。
- 同じ値（枠の比率）が2箇所にあるなら、片方をもう片方から読む。
  べた書きで揃えると、いつか片方だけ古くなる。
- 認証が要る画面は、**全体を開かず対象の関数だけ切り出して**測る。

---

### 9-85. ロゴ完成版へ差し替え／ヘッダーを文字から画像へ（2026-08-14）

ロゴの完成版を受領し、サイト全体を差し替えた。

#### やったこと

1. **アイコン6種をマークのみに統一**（利用者の指定）
   `logo-192` / `logo-512` は「マーク＋TECHNO JAPAN の文字」だったが、
   小さく出る場所で文字が潰れるため全てマークのみに揃えた。
   余白は現行と同じ「横幅の86%」、背景は `#080808`。
2. **ヘッダーの `TECHNO JAPAN` を文字からロゴ画像へ**（452ページ）

#### アイコンには ?v が無く、置き換えても届かなかった

```html
<link rel="icon" href="/favicon.ico">           ← 版番号が無い
```

**ファイルを差し替えるだけでは、ブラウザは古いアイコンを持ち続ける。**
favicon はとりわけ強く保存され、更新が数週間届かないことがある。
`?v=2` を475ファイル（生成側1 + ページ474）と `manifest.json` に付けた。

これは §9-11 で JS / CSS について学んだのと同じ話だが、
**アイコンは検査の対象外だった。**

#### 副作用: 検査が ?v を外さずにファイルを探していた

`?v=2` を付けた途端 `check_regressions.py` が落ちた。

```
参照先が存在しない画像:
  - /images/favicon-192.png?v=2
```

実体はある。検査が `?v=2` ごとファイル名として探していた。
**閾値を緩めず検査側を直した**（版番号を外してから実体を探す）。
実在しない画像を1件埋め込むネガティブコントロールで検知を確認。

#### ヘッダーのロゴ画像化で気をつけたこと

原本は 2726×224（12.17:1）の横長。**そのまま高さを揃えると幅が出すぎる。**
...
マーク部分   0〜428px
文字部分   526〜2726px
文字の高さ  160px（全体 224px の 71%）
```

現在の文字ロゴは 114×16px（実際の字面は約8px）。文字の大きさを
ほぼ保つと全体 158×13px になる。**幅は +44px に収まる。**

幅の上限は実測から決めた。

```
PC(1440px)      右のメニューまで 550px  余裕あり
タブレット(900px) 右のメニューまで  42px  ← ここが効く
```

**PC だけ見ていると衝突に気づけない。** タブレット幅で 42px しか無く、
ここから40px ほど広げると重なる。検査の上限を 190px と決めた根拠。

#### common.css の版が、2箇所で別々に管理されていた

`?v` を上げようとして、既にずれていたことが分かった。

```
詳細ページ 436枚   common.css?v=5   ← 生成側の定数 COMMON_CSS_VERSION
ハブ等      16枚   common.css?v=6   ← HTML にべた書き
```

**同じファイルに2つの版番号が付いて配信されていた。**
今回 7 に揃えた（452枚すべて）。`check_asset_versions.py` は
`common.js` の v=3/4 のずれを警告として出しているが、
**common.css については見ていなかった。**

#### 再発を止める仕組み

`scripts/check_header_logo.mjs` を追加し、preflight の「生成物とデータ」へ。

- 全ページが同じロゴ画像を参照しているか（取り残し 0件）
- 参照先が実在し、`?v` が付いているか
- `width` / `height` 属性があるか（無いと読み込み後にヘッダーが飛ぶ）
- **表示幅が上限 190px 以内か**（タブレットでメニューと重なる線）
- CSS が高さ固定・幅 auto か

ネガティブコントロールは4経路とも検知した。
...
5ページ × 3画面幅（PC / タブレット / スマホ）の15経路すべてで、
表示 158×13px・画面外へのはみ出し無し・メニューとの重なり無しを確認。

#### 教訓

- **画像を置き換えるだけでは届かない。`?v` まで含めて1つの作業。**
  favicon は特に強く保存される。
- **横長のロゴは「高さを揃える」と幅が出すぎる。**
  内部の文字の大きさを基準に決め、上限は狭い画面での実測から引く。
- **PC で余裕があることは、狭い画面で安全なことを意味しない。**
- 同じアセットの版番号を2箇所で持つと、必ずずれる。

---

### 9-86. SNS シェア画像が、283ページで他社フェスの写真だった（2026-08-14）

ロゴ差し替えの調査中に見つけた。**自前の写真を持たないページ283枚の
`og:image` が、Rainbow Disco Club の写真**になっていた。

```
index.html    → images/festivals/rainbow-disco-club.webp
about.html    → images/festivals/rainbow-disco-club.webp
artists.html  → images/festivals/rainbow-disco-club.webp
venues.html   → images/festivals/rainbow-disco-club.webp
news.html     → images/festivals/rainbow-disco-club.webp
```

TECHNO JAPAN のトップページを X や Instagram で共有すると、
**TECHNO JAPAN と無関係の、他社フェスの写真がカードに出ていた。**

#### なぜ気づけなかったか

**ページを開いても分からない。** `og:image` は SNS に貼ったときだけ出る。
サイトを何度見ても、リンクを踏んでも、この間違いは目に入らない。

しかも既定値の宣言そのものは正しく書かれていた。
...
コメントは役割を正しく説明している。**「横長のビジュアル」を探した結果、
手近にあったフェス写真が置かれ、そのまま既定になっていた。**
仮置きが本番になった形。

#### 直したこと

利用者からロゴ入りの専用画像（1200×630）を受領し、
`images/og-default.png` として設置。

- 生成側の `DEFAULT_OG` を差し替え（詳細ページ266枚に反映）
- ハブ15枚は HTML にべた書きだったため直接置換（og:image と twitter:image で30件）
- **RDC 自身のページ2枚は変えない**（本来の画像なので正しい）

#### 再発を止める仕組み

`scripts/check_og_image.mjs` を追加し、preflight の「生成物とデータ」へ。

1. 全ページに `og:image` と `twitter:image` があるか
2. 両者が食い違っていないか
3. 参照先が実在するか
4. **最も多くのページが使っている画像が、特定のフェス／アーティスト／会場の
   写真になっていないか** ← 今回の事故そのもの
5. 既定画像が SNS 推奨の 1200×630（1.91:1）前後か

4 が肝。「既定画像が `/images/festivals/` 配下を指していたら落とす」という
形にしてある。**同じ間違いは、別のフェス写真でも起こりうる。**
...
既定をフェス写真に戻す          → ✅
og:image を1ページ消す          → ✅
twitter:image だけ違う画像に    → ✅
```

#### 教訓

- **画面に出ないものは、見ても分からない。** OGP・JSON-LD・meta の類は
  「開いて確かめる」が効かないので、機械で見るしかない。
- **仮置きは本番になる。** 「とりあえず手近な画像」を既定に置くと、
  誰も気づかないまま283ページに配られる。

---

### 9-87. フッターのロゴも画像に統一（2026-08-14）

§9-85 でヘッダーだけを画像化したため、フッターは文字のまま残っていた。
利用者の指示で451ページのフッターも同じロゴ画像に統一した。

#### ヘッダーと1箇所だけ違えたこと

```html
ヘッダー  <img ... decoding="async">                  ← 即時に読む
フッター  <img ... loading="lazy" decoding="async">   ← 画面下なので遅延
```

フッターは最初の画面に入らないので、先に読み込む必要が無い。

#### 検査が「正しい違い」を不具合と判定した

`check_header_logo.mjs` を拡張したところ、いきなり落ちた。

```
❌ <img> の書き方が 2 種類ある（1種類であるべき）
```

`loading="lazy"` の有無で2種類になっていた。**だがこの違いは正しい。**
ヘッダーとフッターをまとめて「1種類であるべき」と判定していたのが誤り。

**場所ごとに分けて判定**するよう直し、さらに
「フッターには lazy がある」「ヘッダーには無い」を積極的に確かめる検査を足した。
一律に揃えることを求めると、正しい使い分けを壊す方向へ誘導してしまう。

#### 検証

- 4ページ × 3画面幅 = 12経路すべてで、ヘッダー・フッターとも 158×13px
- PC ではフッターのリンク群との間隔 133〜179px、900px 以下では縦積みで衝突なし
- ネガティブコントロール3経路（文字に戻す / lazy を消す / CSS を消す）とも検知

#### 教訓

- **「全部同じであるべき」は、たいてい言い過ぎ。**
  位置が違えば正しい設定も違う。検査は場所ごとに分ける。

---

### 9-88. 共通アセットの版が2箇所で管理され、2回続けてずれた（2026-08-14）

`common.css` と `common.js` の `?v` が、**同じ日に2回、同じ形で**ずれていた。

```
common.css   詳細436枚 v5 / ハブ16枚 v6    （§9-85 で発見）
common.js    詳細436枚 v3 / ハブ16枚 v4    （本項で統一）
```
...
<script src="/common.js?v=${COMMON_JS_VERSION}" defer></script>
```
```html
<!-- ハブ16枚は HTML にべた書き -->
<script src="/common.js?v=4" defer></script>
```

HTML を直しても定数は変わらない。定数を上げても HTML は変わらない。
**どちらか片方を直した人は、直したつもりになる。**

#### 影響

今すぐ何かが壊れているわけではない。だが `sw.js` が JS / CSS を
cache-first で扱うため、次に `common.js` を変更したとき
**436枚には届いて16枚には届かない**（またはその逆）状態になる。
§9-11 と同じ事故の入口。

#### 直したこと

`COMMON_JS_VERSION` を 4 に上げ、452枚すべてを v4 に統一した
（ハブが既に使っていた版に合わせた。内容は同じなので、
逆に v5 へ上げると452枚分のキャッシュを理由なく捨てることになる）。

#### 再発を止める仕組み — ただし「揃えろ」とは言わない

`check_asset_versions.py` には既に「同一アセットに複数バージョン」という
検査があったが、**警告どまり**にしてあった。理由がある。

`detail.css` の v3/v4 は**意図的な分割**で、揃えてはいけない（§9-35）。
フェス詳細178枚だけに効く純粋な追加だったため、新ルールを使わない
264枚は据え置いた。ここを揃えると264枚分のキャッシュを無駄に捨てる。

**だから「混在＝悪」ではない。** 一律に失敗させると、正しい判断を壊す。

そこで狙いを絞った。**生成側が定数を持っているアセットに限り**、
HTML べた書きの版が定数と食い違っていたら失敗させる。
...
定数がある＝「1つの正解がある」という宣言なので、ずれは常に事故。
`detail.css` は定数を持たないため対象外で、§9-35 の分割は壊れない。
...
- **同じ値を2箇所で持つと、必ずずれる。** 同じ日に2回踏んだ。
- **検査は「常に悪いこと」だけを失敗にする。**
  「たいてい悪いこと」を失敗にすると、正しい例外を壊す方向へ人を押す。
  今回は「混在」ではなく「定数とのずれ」に絞ることで両立させた。

---

### 9-89. 受領した SVG がベクターではなかった（2026-08-14）

ロゴのベクター版として `.svg` を受領した。**中身は PNG を包んだだけだった。**

#### 判定の経緯（一度、逆の結論を出した）

1. 中身を数えた: `<path>` 1個 / `<image>` 2個 / `data:image` 1個
   → 埋め込みラスタは 799×417（**今使っている原本と同じ解像度**）
2. 1600px で描画して中間調の割合を比較 → SVG のほうが少なく見えた
   **これは誤り。** 比較した2枚は縦横比が違い、背景の黒が占める割合が
   違うだけだった。**同じ土俵ではない指標を比べていた。**
3. 同じ特徴を切り出して目視 → SVG のほうが鮮明に見えた
   **これも誤り。** 相対座標で切り出したため、比率の違う2枚から
   **別々の場所**を切り出していた。曲率が違えば見え方も違う。
4. 輪郭のにじみ幅を実測 → SVG 2.73px / PNG拡大 2.22px。**差が無い**
5. **決定的な検査**: SVG から `<image>` を取り除いて描画
   → 白い画素 **0.00%**。ロゴが完全に消えた

**見えている絵は、埋め込まれたラスタそのものだった。**

#### 差し替えなかった判断

解像度は現状と同じ 799×417 で、品質は1ミリも上がらない。
むしろ SVG 経由は `feColorMatrix` フィルタを通すぶん描画が重く、
ファイルも 15KB と大きい。**差し替える理由が無い。**

実使用は最大 512px なので、現状の PNG で不足はしていない。

#### 教訓

- **「拡張子が .svg」はベクターであることを意味しない。**
  デザインツールの書き出し設定によっては、ラスタを包むだけになる。
- **比率の違う画像から相対座標で切り出して比べてはいけない。**
  別の場所を見比べて「こちらが鮮明」と判断してしまう。
- 迷ったら**要素を取り除いて描画する。**
  「これが絵の正体か」は、消してみれば一発で分かる。
  間接的な指標を3つ積むより、直接の検査1つが速く確実だった。

---

### 9-90. モバイル監査：大きい画像が原寸で配られていた（2026-08-15）

寝る前の定期セルフ監査として、モバイル（390×844）で12ページを実測した。

#### 見つかった本物の問題

**① 詳細ページの一番大きい画像に srcset が無かった**

```
フェス詳細  原画 1920px(407KB) → モバイル表示 480px（4.1倍）
記事        原画 1920px        → モバイル表示 496px（3.9倍）
```

**関連カードには `cardSrcsetAttr` が付いていたのに、一番重い hero だけ素通し**
だった。派生画像（480w / 960w）は festivals 92枚ぶん既に生成済みで、
出力側で使っていなかっただけ。

**② hero を直したら、次に大きいものが顔を出した**

hero に srcset を入れて測り直すと、今度は**フライヤーが 624KB で最大**になった。
派生は50件すべて生成済み。やはり出力側で使っていなかった。

**「一番大きい画像」を直すと、次に大きいものが最大になる。**
1回測って直して終わりにせず、直した直後にもう一度測る必要があった。

**③ 派生画像が festivals にしか無かった**

`build-image-derivatives.py` の `SOURCE_DIR` が festivals 固定で、
artists 17 / venues 22 / articles 38 の**計77枚に派生が無かった**。
対象を `SOURCE_DIRS` に広げ、169枚 × 2サイズを生成した。

**④ ロゴの当たり判定が 13px だった**

前日ヘッダーを文字から画像に替えた際（§9-85）、当たり判定が画像の高さ
13px まで縮んでいた。文字時代は行の高さで16pxあった。推奨は44px。

`padding` で広げ、**同じ量の負の `margin` で相殺**した。
変更前後をブラウザで実測し、ロゴ画像の上端・左端・ヘッダー高・
メニュー上端の4つが**全て同一**であることを確認している
（幅390 / 1440 の両方）。見た目は1pxも動かさず、当たり判定だけ44pxになった。

**⑤ 地図ページだけ h1 / canonical / hreflang が無かった**

他477枚には全て入っていた。**1枚だけの例外は目視の比較に残らない。**

**⑥ eventAttendanceMode が全 subEvent で未設定**
...
subEvent の構造化データ: startDate / location / eventStatus /
eventAttendanceMode がいずれも **105/105（100%）**。

#### 監査で出した誤検知2件

**⚠️ 測る道具の癖を、対象の欠陥と取り違えた。**

**（a）「フェス一覧のフィルタ11個中6個が押せない」**

最優先バグとして報告しかけた。実際には `.fest-filter-row` に
`overflow-x: auto` があり、スクロールで最後まで到達できる。

原因は私の検査コード。`querySelector('.fest-filter-chip,…,[class*=filter]')`
と書いたため、**`[class*=filter]` が親コンテナ `.fest-filters` に先に当たり**、
チップではなくコンテナの親を調べていた。セレクタの列挙順ではなく
**文書順で最初の一致**が返ることを忘れていた。

**（b）「カスタムカーソルがモバイルでも動いている」**

`common.css:691` に `@media (hover: none) and (pointer: coarse)` で
非表示にする指定が**既にあった**。headless Chrome がタッチ端末を
模擬しないため `display: block` に見えていただけ。

**どちらも「実測した」つもりで、測定条件が実機と違っていた。**

#### 直さなかったもの — 既存の判断を尊重した

ハブのカード画像に srcset が無い。一見「最適化漏れ」だが、
`localize.js:71` に **「一覧ハブでは使わないこと」** と明記があった。

> カードは実測でモバイル452px / PC 848px と大きく、960px がちょうど良い。
> srcset を付けると PC で 480px が選ばれて画質が落ちた（2026-08-07 実測）。

実機は dpr 2〜3 なので、452px のカードには 960px が適正。
**私の計測は dpr=1 で、実機より小さい画像で足りるように見えていた。**
コメントを読まずに「直して」いたら、画質の回帰になっていた。

#### 教訓

- **一番大きいものを直したら、もう一度測る。** 次に大きいものが最大になる。
- **セレクタは文書順で最初の一致を返す。** 列挙順ではない。
  `[class*=…]` のような広いセレクタは、意図しない親に先に当たる。
- **headless は実機ではない。** タッチも dpr も既定では模擬されない。
  「実測した」と言う前に、測定条件が実機と同じかを確かめる。
- **既存コードの「やらない」というコメントは、たいてい実測の結果。**
  最適化漏れに見えるものほど、まず理由を探す。

---

### 9-91. 記事本文の画像が 2000px で配られていた／「実測した」の中身が違っていた（2026-08-15）

利用者から「記事画像に alt が無い影響は？」と聞かれ、範囲を数え直したところ
**8枚ではなく46枚**（日英の全記事）だった。前回の報告は1ページ分だけの数字で、
過少だった。

そして調べる過程で、**alt よりはるかに重い問題**が出た。

#### 本文画像が 2000px で配信されていた

本文の画像は CMS から Google Drive のURLで入っており、末尾が `=w2000`。

```
実際の表示幅   スマホ 484px / PC 503〜1332px
配信           2000px 指定 = 1枚あたり最大 628KB
記事1本に8枚
```
...
#### 前夜の計測が、この 3MB を数えていなかった

§9-90 で「記事 501KB → 209KB」と報告した。**実際は約3.2MBだった。**

計測用のサーバーが**自サイトへの通信しか数えておらず**、
Drive への通信は素通りしていた。数字は正しく出ていたが、
**測っている対象が利用者の受け取るものと違っていた。**

`performance.getEntriesByType('resource')` をブラウザ側で読む方式に変え、
外部ドメインも含めて測り直した。
...
| **スマホ 390px dpr3（実機相当）** | 3,539KB | **1,836KB** | −48% |
| PC 1440px dpr2 | 3,539KB | 1,836KB | −48% |

#### src は変えず srcset だけ足した

src を変えると `IMAGE_DIMENSIONS` の引き当てキーが変わって
`addHtmlImageDimensions` が落ちる。**srcset を足すだけ**にすれば、
解釈する現代のブラウザは小さい方を選び、解釈しない環境は従来どおり src を読む。

#### 候補に 1600 を足したら、悪化したので戻した

`fx-full`（全幅表示）の画像が PC Retina で画素不足になるため、
候補に 1600 を足して測った。

```
実機相当（390px / dpr3）  1,836KB → 2,646KB（+44%）
```

得られるのは **PC の全幅画像を Retina で見たときの精細さだけ**で、
モバイル優先の方針に見合わない。**入れずに戻した。**
測らずに入れていたら、改善のつもりで悪化させていた。

#### Drive は原本より大きくは返さない

計測中に分かったこと。原本 1100px の画像は `=w1200` も `=w2000` も
同じ 1100px・208KB を返す。**「w2000 だから1920px」とは限らない。**
画像ごとに原寸が違うので、URL の数字を見ても実際の重さは分からない。

#### alt が無いことの影響（利用者への回答）

46枚すべてに `alt` が無い。影響は4つ。

1. **読み上げソフトがURLを読む。** `alt` が「無い」場合、ファイル名やURLを
   読もうとする。`alt=""`（空）と書けば「装飾なので飛ばす」という指示になり
   無視される。**「無い」と「空」は別物**で、現在は最悪の状態。
2. **画像が出ないとき何も残らない。** 通信不良や Drive 側の障害で
   空白の四角が並ぶ。
3. **画像検索からの流入を落とす。**
4. **AI 検索が写真の内容を把握できない。** ChatGPT / Perplexity は
   `alt` を通してしか画像を理解しない。写真が主役のメディアでは効く。

単独では緊急ではないが、記事が増えるほど増える。
**CMS 側で入力時に必須化するのが本筋**で、今回は未対応。

#### 教訓

- **「実測した」の中身を確かめる。** 数字が出ていることと、
  利用者の受け取るものを測れていることは別。
  外部ドメインは自前のプロキシでは数えられない。
- **良くする変更でも、悪化していないか測る。** 1600 の件は
  測ったから戻せた。
- **URL のサイズ指定を信用しない。** 原本より大きくは返らない。

---

### 9-92. 画像の説明文（alt）を CMS で入力できるようにした（2026-08-15）

§9-91 で「本文の画像46枚すべてに alt が無い」と分かった。
利用者から「CMS で入力できる形にするのが先か」と問われ、調べたところ
**その順でなければ定着しない**ことが確認できた。

#### なぜ CMS が先なのか

本文の HTML はスプレッドシートの `BODY` 列にある。生成側で alt を足しても、
**次の Publish でシートの中身に上書きされて消える。**

```
入力欄を作る → シートに保存される → 生成物に出る
```
この順でないと残らない。

#### 受け皿はほぼ出来ていた

記事編集画面には**本文の画像をクリックすると出る設定パネル**が既にあり、
`data-layout` / `data-crop` / `data-zoom` / `data-x` / `data-y` を書き込む
流れができていた。さらに `cms.js:1237` / `1255` に
**既存の alt を保持する処理**まで入っていた。

**「入力できない」のは、入力欄が無いだけだった。** 追加したのは3箇所:

1. `cms.html` に `#ar-image-alt`（入力欄）
2. 画像を選んだときに現在の alt を読み込む
3. 「画像に適用」で `setAttribute('alt', ...)`

#### 空にしても属性は残す

`alt` が**無い**と読み上げソフトは URL を読もうとする。
`alt=""` と書けば「装飾なので飛ばす」という明確な指示になる。
**「無い」と「空」は別物**なので、空欄で適用しても属性ごとは消さない。

#### 検証：実物の Quill で往復させた

CMS は認証で headless が止まるため（§9-84 と同じ）、
**同梱の Quill を読み込み、cms.js から実物の関数を切り出して**動かした。

```
✅ 「画像に適用」で alt が入る
✅ 保存されるHTMLに alt が含まれる
✅ 閉じて再表示しても消えない（別の Quill に流し込む経路）
✅ 再表示時に入力欄へ読み戻る
✅ 空にしても alt="" が残る（属性ごと消えない）
```

生成側（`addDriveImageSrcset` / `addHtmlImageDimensions`）が alt を
消さないことも、実物の関数を切り出して4パターンで確認した
（説明文あり / 空 / なし / 属性が前後にある）。

#### 検査は「空でない alt」で数える。ただし止めない

`alt=""` を合格にすると、一括で空を入れて検査を通し、そのまま忘れる。
**残りが見え続ける状態を保つ**ため、空は未入力として数える。

一方で、46枚を人が書くまで preflight を赤にし続けると
無関係な作業まで止まる。**当面は警告として枚数を出すだけ**にした。

```
⚠️  説明文の入力: 0/46枚（属性なし 46枚 / 空 0枚）
```

#### ネガティブコントロールで検査の欠陥が出た

「有効化の配列から外す」を検知できなかった。原因は検査側。

```js
/ar-image-alt[^\n]*\]\.forEach/.test(cmsJs.replace(/\s+/g, ' '))
```

**空白をまとめた結果ソース全体が1行になり、`[^\n]*` が全文に広がった。**
配列から外しても、別の場所の `ar-image-alt` を拾って通ってしまう。
配列そのものを取り出して中身を見る形に直した。

#### 並行作業への配慮

Codex が同日 `build-detail-pages.mjs` を編集していた（11:41）。
**生成側に触れず CMS 側だけで完結**するよう設計した。
着手前に「再生成しても差分ゼロ」を確認し、以降の差分が自分のものだけに
なるようにしている（AGENTS.md の並行作業の項）。

#### 教訓

- **入力欄が無いだけ、ということがある。** 保持する処理まで書かれていた。
  「機能が無い」と決める前に、どこまで出来ているかを確かめる。
- **`alt` の「無い」と「空」は別物。** 空は「飛ばせ」という指示。
- **正規表現で空白をまとめると、`[^\n]*` の意味が変わる。**

---

### 9-93. ニュースの記事画像が原寸で配られていた——対応表の読み込み漏れ（2026-08-19）

定期監査（2回目）。前回（§9-90）と同じ方法で実測し、1件の本物と、
健全性の確認がいくつか。

#### ニュースとトップの記事画像が原本を読んでいた

実機相当（390px / dpr3）の実測:

```
ニュース  2,157KB（うち画像 1,808KB）
トップ    1,531KB（うち画像   695KB）
snow-mashine-2027-info.webp = 686KB が両方に載っていた
```

記事カードの描画が `tjAssetPath`（原本）を使っていた。派生（縮小版）は
§9-90 で38枚生成済み。フェス一覧は `tjCardAssetPath` で派生を使っており、
**記事カードだけが規約から漏れていた。**

#### 直したら半分しか直らなかった——本当の原因は読み込み漏れ

4箇所を `tjCardAssetPath` に直したが、**トップだけ改善してニュースは
1バイトも変わらなかった。**

```
news.html だけ image-derivatives.js（対応表）を読み込んでいなかった
```

`tjCardAssetPath` は対応表が無いとき**エラーを出さず原本へフォールバック**
する。表示は完全に正常なので、重さを測らない限り気づけない。
このサイトが繰り返し踏んできた「静かな失敗」の典型形。

#### 結果

```
ニュース  2,157KB → 869KB（−60%）
トップ    1,531KB → 1,034KB（−32%）
英語版も同じ
```

表示の健全性も確認: 5経路（ニュースSP/PC・トップSP/PC・英語ニュース）で
壊れ画像0・非表示化0・派生使用9枚。

#### 再発防止

`check_image_delivery.mjs` に「`tjCardAssetPath` を使う HTML は
`image-derivatives.js` を読み込んでいること」を追加。
ネガティブコントロール（news から読み込みを外す）で検知を確認。

フォールバック付きの関数は安全に見えて、**前提が欠けたときに静かに
劣化する**。フォールバックには検査を対にすること。

#### 前回からの健全性（問題なし）

- 横スクロール0 / JSエラー0 / 404ゼロ / h1 全ページ1個（地図含む）
- Codex のホバープレビュー（8/18）は `hover:none` / 1100px 以下で
  正しく無効化されており、モバイルに影響なし
- alt 入力: 0/46 → **16/58**（利用者が入力中。分母は新記事で増加）
- 英語記事: CMS の一括生成（8/18）で 5本が公開済み

---

### 9-94. ハブ検査が共有マシンの過負荷で不安定になり、無効な関門で2回 push した（2026-08-19）

#### 起きたこと

preflight のハブ描画検査が、**毎回違うページ**で落ちるようになった。

```
1回目  en/festivals.html
2回目  news.html + en/artists.html
3回目  submit.html + en/festivals.html + en/artists.html
症状は全て同じ:「DOMが空」（0個・0B・コンテナ不明の三点セット）
```

原因はコードではなく環境。負荷平均が 20〜35 に達しており
（並行セッションの作業＋実ブラウザ＋**私の計測が残したゾンビ headless Chrome 13個**）、
headless の描画が時間切れで空の DOM を返していた。
本物の描画故障と、時間切れの空返りが、検査上で同じ顔をしていた。

#### 検査を負荷に耐える形にした

`check_hub_pages.py` に「**DOM がほぼ空（<2KB）のときだけ、その場で
1回・倍の猶予で再試行**」を追加。ネガティブコントロールで安全性を確認:

```
本物の故障を再現（カードのクラス名を破壊。DOM は 131KB 返る）
→ 再試行があっても markers 0 で検出、exit=1 ✅
```

救済されるのは「何も返らない」だけ。JS エラーでカードが出ない類の
本物の故障は DOM 自体は返るため、従来どおり落ちる。

#### もう1つの問題: パイプが exit code をすり替え、無効な関門で2回 push した

```
bash preflight.sh | grep ...   ← 終了コードは grep のもの
python3 check.py | tail -16    ← 終了コードは tail のもの
```

「exit 0 だった」と判断した2回は、どちらも**パイプ末尾のコマンドの
終了コード**を見ていた。同じ形を1日で3回踏んだ。
push した中身はいずれも無害（docs / スクリプトのみ）だったが、
「検査が通ってから進む」の関門としては無効だった。

#### 運用の固定

1. **検査はパイプに通さない。** 単独実行し、出力はファイルへ、
   exit code は `echo "exit=$?"` でログに直接記録する
2. push は検査の exit code を確認した**後続コマンド**で行う
   （1つのコマンド内で無条件に連結しない）
3. 長い検査は最初から `run_in_background` で流す
   （途中で timeout に殺されると headless Chrome が孤児として残り、
   それが次の検査を落とす——今回の13個はこうして溜まった）

#### 教訓

- **検査の失敗には「対象の故障」と「測定環境の故障」がある。**
  毎回違う場所が落ちるのは後者のサイン。
...
### 9-96. 本番が10日間更新されていなかった——失敗ではなく、気づかなかったことが事故（2026-08-24）

#### 起きたこと

2026-08-14 から 08-24 まで、**本番が10日間更新されていなかった。**

気づいたのは偶然だった。別件（CMSの公開確認ダイアログの検証）で
`main` の `LP/data.js` と本番の `data.js` を比べたところ、内容が違っていた。
...
`main` に公開済みのはずのものが、本番へ届いていなかった。

#### 10日間の内訳

Publish pipeline の履歴（**時刻はUTC。gitのコミット時刻はJSTなので +9時間ずれる**）。

| UTC | 結果 | 理由 |
|---|---|---|
| 08-14 05:40 | success | ← 最後の成功 |
| 08-17 10:07 | failure | `EDITIONS: ID重複 "snow-machine-japan-2026"` |
| 08-17 13:54 | failure | `cms.js` の参照が `?v=82` のまま（キャッシュ版数の更新漏れ） |
| 08-17 14:18 | failure | `broken_image_refs 2 > max 0`（mutek-flyer.webp / mutek.webp が不在） |
| 08-23 17:38 | failure | `EDITIONS: ID重複 "grow-the-culture-open-air-2026"（42行目と109行目）` |

**失敗理由が毎回違う。** ひとつを直しても、次は別の理由で落ちた。
そして 08-24 02:25〜02:36 UTC の `cms: publish data.js` 3件（`bc837416` /
`207eac82` / `497e0354`）は**空コミット**で、`paths: LP/data.js` に一致せず
**ワークフローが起動すらしなかった**。

※ `075fb6d2` は空コミットではない（1ファイル152行追加）。JST 08-24 02:37 =
UTC 08-23 17:37 で、上表の 08-23 17:38 の失敗runがこれにあたる。
handoff に一時「空コミット3件」に含めて書いたが誤りなので、ここで正す。

最後に発見時点で公開を止めていたのは、また別の理由だった。
`build-detail-pages.mjs` の `lineups.json: ARTIST_ID 参照切れ`。
ARTISTS シートの Joma / Noritake が `draft` になり、data.js から外れたのに、
LINEUPS の `matricaria-2026` が両名を参照したまま残っていた。

#### 事故の本体

**5回の失敗そのものではない。10日間、誰もそれを知らなかったことである。**

検査は正しく落ちていた。ID重複も版数の漏れも画像欠損も参照切れも、
すべて仕組みが検出していた。**検出結果が人に届く経路が無かった。**

さらに悪いことに、症状は「Publish Now を押しても何も起きない」という
**沈黙**だった。押した側には成功とも失敗とも見えない。
§9-58 / §9-66 / §9-67 / §9-72 / §9-80 と、Publish の事故はすべて静かだった。
今回はその極端な形で、静かなまま10日続いた。

#### 直したこと

データ側はユーザーが対応した（Joma / Noritake を公開に変更）。
その後 Publish Now を1回実行し、10日ぶりに pipeline が成功した。
以後 3回連続で成功している。

再発防止として **Production sync watchdog** を入れた
（`scripts/check_production_sync.mjs` / `.github/workflows/production-sync-watchdog.yml`）。

見るのは Actions の成否だけではない。**本番が実際に配っているもの**を
`main` と SHA-256 で突き合わせる。**ワークフローが起動しなかった事故は、
成否だけ見ていても永久に見つからない。**今回の空コミットがまさにそれだった。

30分ごとの定期実行に加え、Publish pipeline / sitemap生成の失敗で即時発火する。

#### 誤報を出さない設計にした理由

Publish から本番反映まで数分かかる。その間 `main` と本番は**正しく**食い違う。
ここで毎回鳴らすと、通知は「またか」と読み飛ばされるようになる。
**そうなった時点で、この見張り番は今回の事故を防げなくなる。**
気づかれない通知は、無い通知と同じだからである。

そこで不一致を見つけたときだけ追加で確認する。

- `publish-pipeline` / `deploy-pages` に完了していない run があるか
- `LP/data.js` の最終更新が10分以内か（pipeline の実測は1分40秒前後）

どちらかなら異常として扱わない。ただし **API を引けないときは猶予を与えず鳴らす。**
見逃すより誤報の方がまだ直せる。

#### 検知が壊れても緑にならないようにした

当初の自己テストは `sha256` と run 抽出だけで、**不一致検知そのものを通っていなかった。**
検知が壊れても preflight は緑のままになる。それでは §9-82（`.json` の中の `.js` を
壊していた）と同じ穴になる。

判断を純関数 `evaluateSync` に切り出し、preflight から15項目を検査する。
「不一致で落ちる」「一致で通す」「公開中は鳴らさない」「10日前は猶予外」まで含む。
`evaluateSync` の戻り値を `ok:true` に改変すると自己テストが exit 1 になることを実測した。

**`evaluateSync` は純関数のまま保つこと。** ネットワークを混ぜると自己テストが書けなくなる。

#### 見張り番は実装直後に1件検知した

`Generate sitemap.xml & rss.xml` の最新runが 08-24 03:51 UTC の failure のままだった。
誤検知ではない。手動再実行で解消した（中身は問題なく、本番sitemapは446URL・
Joma / Noritake 掲載済み・main と一致していた）。

#### 教訓

- **押した人に結果が見えない操作は、いつか静かに壊れる。**
- **検査が落ちることと、人が気づくことは別の問題である。** 前者はずっとできていた。
- **本番が実際に配っているものを見る。** 中間状態（コミットの有無、runの成否）だけを
  見ていると、経路そのものが動かなかった事故を取りこぼす。
- **誤報を減らすことは、機能ではなく前提である。** 信用されない通知は存在しないのと同じ。

### 9-97. LINEUPS が保存のたびに末尾へ増殖していた（2026-08-27）

#### 起きたこと

CMS でフェスの出演者を編集・保存すると、LINEUPS シートに同じアーティストが
何重にも積まれた。mutek-2025 は同一アーティストが**5行**（= 同一画面で5回保存）。
発見時点でシート全体の完全重複は **258行**（8/24 時点の192行から3日で66行増。
現在進行形の事故だった）。重複は本番のフェス詳細の LINE UP にそのまま表示されていた。

#### 原因は2つの取りこぼし

§9-58 で EDITIONS の保存は「ID があれば上書き（upsert）」に直された。
そのとき **LINEUPS は直っていなかった**。

1. **新規扱いの開催回のラインナップは、無条件で末尾追記**だった。
   EDITIONS 行は upsert されるのに、その出演者行は毎回まるごと追記された。
2. **保存に成功しても `e._row` が付かなかった。**そのため同じ画面で保存し直すたびに
   その開催回は「新規」と判定され続け、1. の追記が保存回数ぶん繰り返された。

さらに既存開催回の経路（`syncExistingEditionRows`）は「行数が同じ」前提の
1:1 上書きで、**アーティストを追加すると黙って消え、削除すると古い行が残った**。
ユーザーが「消えた」と再保存を繰り返すほど、1.2. の増殖が進む構図だった。

#### 直したこと（3層）

1. **CMS（cms.js）**: `syncLineupRowsForEdition()` に一本化。
   既存行を上書き → 余り行は**空白化** → 不足分だけ末尾追記。
   行削除はしない（delete_row は以降の行番号を全てずらし、別の事故になる）。
   保存後は `e._row` を付け、セッション台帳（lineupSheetRows）も更新する。
   重複が積まれた開催回は**保存するだけで自然に掃除される**設計。
   ARTIST_ID はラベルが変わっていない行だけ引き継ぐ（並び替えで別人のIDが付く事故防止）。
2. **配信パイプライン（fetch-data.mjs）**: 全列一致の完全重複行を除去してから
   lineups.json を書く。何行落としたかは必ず警告に出す（黙って直さない）。
   初回実行で 993 → 735 行（258重複）を実測。同じアーティストの正当な複数出演
   （別ステージ・別日・b2b）はどこかの列が違うので消えない。
3. **検査（check_cms_publish_guard.mjs / preflight）**: 「重複シートを1回の保存で
   掃除できる」「同じ内容の再保存で行が増えない」「追加は追記・削除は空白化」
   「ラベル変更行に ARTIST_ID を引き継がない」を毎回検証する。

#### 追記（同日）: さらに深い根本原因 — get_sheet のキーは小文字

上の2つの取りこぼしを直した後、シート掃除の準備で決定的な事実が見つかった。

**GAS `get_sheet` は列名を小文字で返す**（`EDITION_ID` → `edition_id`）。
一方 `loadEditionsFromSheet` とその先の照合はすべて `r.EDITION_ID` と大文字で読む。
つまり:

- `editionSheetRows` のフェス絞り込み（`r.FESTIVAL_ID`）は**常に空振り**
  → CMS の開催回は常にフォールバック（FESTIVALS の JSON）から表示され、
  **`_row` を持たない = 全開催回が保存のたびに「新規」扱い**だった
- §9-58 の upsert（`editionRowById`）も**空の Map と照合していた**
- §9-66 対策で入れた publish 前の EDITIONS 重複ガードも `r.EDITION_ID` を読んで
  **空振り**（8/17 の snow-machine・8/23 の grow-the-culture が素通りした理由）
- **§9-96 の10日間障害で繰り返された「EDITIONS ID重複」失敗も、源流はここ**

さらに皮肉なことに、§9-69 で作った `canonicalizeRows`（取得経路によるキーの
大小差を吸収する仕組み）は存在していたが、**別名リストに EDITIONS / LINEUPS の
大文字列名が入っておらず**、`loadEditionsFromSheet` はそもそも呼んでいなかった。
コメントに「経路が変わるだけで値が黙って消える」と書いた本人たちが、
その2行下で同じ罠を踏んでいた。

対応: 別名リストへ大文字名24個を追加（1つの列に複数の正しい綴りを許す形へ拡張）し、
`loadEditionsFromSheet`・publish の EDITIONS/LINEUPS 取得すべてで
`canonicalizeRows` を通した。検査には**実物と同じ小文字キーの行**を使う
回帰テストを追加した（大文字のテストデータでは、まさにこのバグを検出できない）。

計測時の自戒: この調査でも当初 `r.EDITION_ID` でシートを読み、
「重複0件」という誤った測定を2回出した（実際は260行）。
**シートを読む前に、1行の実キーをダンプして確かめること。**

#### 教訓

- **upsert 化は「その行に紐づく子行」まで含めて完了である。** §9-58 は親（EDITIONS）
  だけを直し、子（LINEUPS）が同じ病気のまま残った。
- **保存成功後の状態更新を忘れると、2回目以降の保存が別の経路を通る。**
  「1回目は正しいのに繰り返すと壊れる」症状はここを疑う。
- 「追加したのに消える」というユーザー体験は、再保存の連打を誘発して
  別の欠陥（増殖）を加速させる。
- **防御機構は「実物のデータ形状」でテストしない限り、動いている証明にならない。**
  §9-58 / §9-66 の対策は大文字キーのテストでは緑、実データでは空振りだった。
- ガードが空振りしても誰も気づかない。**「ガードが発火した実績」を時々確かめる**
  （一度も発火していないガードは、壊れているガードと見分けがつかない）。

### 9-98. 見張り番が「消せないゾンビ」に12日間、空振りの警報を鳴らし続けた（2026-08-31）

#### 起きたこと

「Unstick CI queue」（詰まった CI を外す見張り番 / §9-65）が、8/19 から
15分ごとに失敗し続けていた（直近100回中42回が赤）。ユーザーが
「GitHub のエラーが定期的に来るが、致命的か？」と気づいて発覚した。

**本番への実害はゼロだった。**この間、デプロイは全件成功し、
Production sync watchdog も緑（8/30 06:10 の1回だけ通信エラーで赤、次回から復帰）。

#### 原因

8/19 の「Deploy LP to GitHub Pages」1件が、GitHub 内部で壊れたまま
`queued` 表示で残留した（12日間 = 17,000分超）。この run は

- 通常の cancel も force-cancel も **409** で断られる。文言は
  「Cannot cancel a workflow run that has not been queued yet」
  （queued と表示されているのに「まだ queued になっていない」と言われる）
- **後続は塞がない。**concurrency: pages の列には実質的に居ない

つまり API からは永久に消せず、かつ無害なゾンビ。ところが見張り番は

1. 毎回これを「詰まり」として検出し、キャンセルに失敗（hard 扱い）
2. HEAD に workflow_dispatch のデプロイ実績があるため「蹴り直したのに
   また詰まった」という §9-65 の無限ループ警報を誤発動して exit 1

「蹴ったデプロイ」は実際には毎回成功していたのに、警報の文言は
「また詰まりました」だった。**警報の条件（dispatch 実績の有無）と
文言（また詰まった）が一致していなかった。**

#### 直したこと

`classifyCancelFailure()` を純粋関数として切り出し、キャンセル失敗を3分類にした。

| 分類 | 条件 | 扱い |
|---|---|---|
| transient | HTTP 5xx | 保留（従来どおり） |
| phantom | 409 + 「has not been queued yet」 | **保留を追加**（今回のゾンビ） |
| hard | それ以外（403 等） | 失敗として報告（従来どおり） |

1件も外せず、かつ全てが transient/phantom のときは、デプロイの蹴り直しも
警報もせず警告 Summary だけ残して緑で終える。実事故の 409 文言を使った
回帰テストを check_unstick_queue.mjs に追加した（「completed への通常 409」は
hard のままであることも含む）。

ゾンビ run 自体は消せないので放置する。GitHub 側の掃除で消えるのを待つ。

#### 教訓

- **見張り番は「直せない異常」を検出したとき、鳴り続ける以外の道を持つこと。**
  15分ごとの赤通知が12日続くと、本物の警報が埋もれる（§9-96 と同型の
  「通知疲れ」を、今度は見張り番自身が作っていた）。
- **警報の条件と文言を一致させる。**「また詰まった」と言うなら、蹴り直した
