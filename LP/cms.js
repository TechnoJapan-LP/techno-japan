...
function bindArticlePreviewInteractions(root){
  ...
  root.addEventListener('click', event => {
    ...
    let scrollTarget = target.previousElementSibling;
    while (scrollTarget && !/^H[234]$/.test(scrollTarget.tagName)) {
      scrollTarget = scrollTarget.previousElementSibling;
    }
    scrollTarget = scrollTarget || target;
    if (scroller) scroller.scrollTop = Math.max(0, scrollTarget.offsetTop - 24);
