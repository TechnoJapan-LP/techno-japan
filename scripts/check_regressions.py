...
def measure():
    ...
    # Skipリンクがタップ後に残らず、固定ナビがWebViewでも上端に留まることを確認する。
    common_css = read(LP / "common.css")
    mobile_browser_guards = int(
        bool(re.search(r"\.skip-to-content:focus-visible", common_css))
        and bool(re.search(r"\.skip-to-content\s*\{[^}]*display:\s*none\s*!important", common_css, re.S))
        and bool(re.search(r"nav\s*\{[^}]*position:\s*sticky\s*!important", common_css, re.S))
    )
