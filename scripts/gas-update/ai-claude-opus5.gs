...
/* リード文・メタ説明・タイトル3案そのものは短い。だが **max_tokens は
   「出力の上限」であって「本文の長さ」ではない。**モデルが考える過程を
   出力に含めると、本文に辿り着く前に上限へ当たり、**本文が空で返る。**

   2026-08-10、翻訳（上限16000）は成功するのにタイトル生成（上限1000）だけが
   空になった。差はこの数字だけだった。AUDIT §9-73。

   上限は「ここまで使ってよい」という枠で、**課金は実際に出力した分だけ。**
   余裕を持たせても費用は増えない。逆に切り詰めると、上のような形で静かに壊れる。 */
var MAX_TOKENS_SUMMARY = 4000;
