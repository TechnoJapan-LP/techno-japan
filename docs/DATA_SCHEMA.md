...
**TAGS は現時点で未稼働(2026-08-02 調査)**: FESTIVALS / VENUES / ARTISTS の TAGS は入力UIがあるがサイトへ出力されない。値を入れても無駄になるため、下記が整うまで入力しないこと。実データも全件空(記事のみ値あり)。

活用する場合の前提:

1. **Publish 経路に載せる** — `buildFestivalsJs` / `buildVenuesJs` / `buildArtistsJs` は `tags` を出力しない。出力しているのは `buildArticlesJs` のみ。
2. **CITY の表記を揃える** — CMS の「＋ ジャンル/都市から候補追加」は GENRE + CITY + TYPE をタグ候補にするが、FESTIVALS は `Chiba`、VENUES/ARTISTS は `TOKYO` と大小が不統一。同じ東京から `Tokyo` と `TOKYO` の2タグが生まれる。§1.5 の PREF 規約へ正規化してから使う。
3. **`addTag()` に正規化を入れる** — 現状は完全一致で重複判定するだけで、大小変換もしない。表示は原文のまま出る。
4. **タグの表記規約をこの文書に定める** — GENRE には §1.3 の正規リストがあるが、タグ全体の規約は未定義。大小・言語・区切り・許可語彙を決める。日本語タグの前例は現時点でゼロ。

着手する順序は **4 → 2 → 3 → 1**。逆順にすると、揺れたタグが本番に出てから直すことになる。
...
> 節の前置きではなく**各表の直前**に印を置いている（実際に、この警告を
> 読まないまま目標の `EDITIONS.LOCATION_JA` を実在の列として扱った誤りが
> 2026-08-03 に発生した。AUDIT §9-32）。
