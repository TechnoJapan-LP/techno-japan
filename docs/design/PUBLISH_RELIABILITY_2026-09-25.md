...
| 分類 | 件数 | 日付 | 具体例 |
|---|---|---|---|
| **A. 画像の同期タイミング** | 6 | 09-25, 09-20, 09-13, 09-12, 09-09, 08-17 | 参照画像が未同期 / 3比率の派生が未生成（09-25・09-13 は同型）/ Sync が起動できず |
...
**A + B + C で 14 / 16 件。**（当初 09-13 を「外部 API の一時障害」と読み違えたが、ログの ❌ 行は 09-25 と同じ3比率不足だった） この3つを構造的に潰せば「毎回エラー」は止まる。
...
### M5.（取り下げ）外部 API の検査除外

09-13 の失敗は Claude API ではなく 09-25 と同じ「3比率の派生不足」だった（ログの ✅ 行に含まれる
"Claude API 502" はモック検査の期待値）。`check_gas_ai.mjs` は本物の API を呼んでいない。対策不要。
...
| 4 | M3 CMS 関門 | `LP/cms.js` | **触る（CMS）** | 実機 Publish＋CMS 実操作（AGENTS.md 手順）。最後に回す |

## 効果の見込み

| 分類 | 対策後 |
|---|---|
| A 画像 6件 | **0**（M1。原本未同期は既存の自動 Sync が対応） |
