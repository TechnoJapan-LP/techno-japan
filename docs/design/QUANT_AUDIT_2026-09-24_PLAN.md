...
| WP | 状態 | 結果 |
|---|---|---|
| WP1 | **完了・本番反映**（`86f9551f`） | タップ 62→0、コントラスト 344→10（本番実測）。残りは写真上の文字等 |
| WP2 | **完了・本番反映**（`dba4ffc4`） | onclick 1,114→0、onerror 8→0。メニュー開閉・画像フォールバックを本番で確認 |
| WP3 | **完了・push 済**（`f5fcc8fd`） | 1MB超 3→0（949/961/961KB）。次回 Sync Drive Images のログで `Recompressed` を要確認 |
| WP4 | 保留 | `search.js` にコメント1行を足すだけで 546 ページの `?v` が上がる。次に JS を触るときに同乗させる |
| WP5 | 保留 | 未使用17クラスのうち `ql-*` 4つは CMS（Quill）が将来出力しうるので消せない。残り13は次の CSS 変更に同乗 |
| C1〜C3 | **ユーザー作業（CMS）** | 見出し階層 7記事 / ultra-japan チケット URL / triangle 公式 URL / synapse 記事の画像 alt 3枚 |
